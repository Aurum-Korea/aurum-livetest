...
function LaunchCalc({ activeTier, setActiveTier, prices = { gold: 3342.80 }, krwRate = 1368 }) {
  ...
  // BUG FIX #3: compute tIdx directly — never call setState during render
  const tIdx       = tierIdx >= 0 ? AGP_TIERS.length - 1 - tierIdx : 0;
  const tier       = AGP_TIERS[tIdx];
  const pct        = ((monthly - 200000) / (5000000 - 200000) * 100).toFixed(1);
  const fmt        = n => Math.round(n).toLocaleString('ko-KR');
  // Sync activeTier for Ingot visual via useEffect (NOT during render)
  useEffect(() => { setActiveTier(tIdx); }, [tIdx]);
...
function GMVBonusTrack({ navigate }) {
  return (
    <div style={{ background:T.bgCard, border:`1px solid ${T.goldBorder}`, padding:'28px 28px', height:'100%', position:'relative', overflow:'hidden', display:'flex', flexDirection:'column' }}>
      ...
      {/* FIX: fontSize:12→13 */}
      <p style={{ fontFamily:T.sansKr, fontSize:13, color:T.textSub, lineHeight:1.95, marginBottom:22 }}>
...
// ─── 5-Tier cards ─────────────────────────────────────────────────────────────
function TierCards({ activeTier, setActiveTier, lang = 'ko' }) {
    ...
    <div>
      ...
      {!isMobile && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 12, marginBottom: 4 }}>
          {[
            { col: ko ? '레벨' : 'LEVEL', align: 'center' },
            { col: ko ? '월 적립액' : 'MONTHLY', align: 'center' },
            { col: ko ? '론치 기프트' : 'LAUNCH GIFT', align: 'center' },
            { col: ko ? '혜택' : 'PERKS', align: 'center' },
            { col: '', align: 'center' },
          ].map((h, i) => (
            <div key={i} style={{ fontFamily: T.mono, fontSize: 9, color: T.textMuted, letterSpacing: '0.18em', textTransform: 'uppercase', textAlign: h.align, padding: '0 16px 6px', borderBottom: `1px solid rgba(197,165,114,0.15)` }}>
              {h.col}
            </div>
          ))}
        </div>
      ...
      {AGP_TIERS.map((tier,i)=>{
        ...
        return (
          <div key={i} onClick={()=>setActiveTier(i)} style={{ gridColumn:isMobile&&tier.featured?'1 / -1':'auto', background:tier.featured?`linear-gradient(180deg,${T.goldGlow},${T.bg})`:T.bg, border:`1px solid ${isActive?T.goldBorderStrong:tier.featured?T.goldBorder:T.border}`, padding:isMobile?(tier.featured?'24px 20px':'20px 12px'):'28px 16px', textAlign:'center', position:'relative', overflow:'hidden', cursor:'pointer', transform:isActive?'translateY(-4px)':'none', boxShadow:isActive?`0 10px 32px rgba(197,165,114,0.15)`:'none', transition:'all 0.3s cubic-bezier(0.2,0.8,0.2,1)' }}>
            ...
            <div style={{ fontFamily:T.mono, fontSize:10, letterSpacing:'0.16em', textTransform:'uppercase', color:T.textMuted, marginBottom:4 }}>{ko ? '월 납입액' : 'MONTHLY'}</div>
            <div style={{ fontFamily:T.mono, fontSize:13, color:isActive?T.goldBright:T.textSub, fontWeight:600, marginBottom:14 }}>{tier.monthly}</div>
            <div style={{ fontFamily:T.mono, fontSize:10, letterSpacing:'0.16em', textTransform:'uppercase', color:T.textMuted, marginBottom:5 }}>{ko ? '론치 기프트' : 'LAUNCH GIFT'}</div>
        ...
        {steps.map((row,i)=>(
          <div key={i} style={{ display:'grid', gridTemplateColumns:'48px 1fr', alignItems:'start', padding:'16px 0' }}>
            ...
            <div>
              ...
              <div style={{ fontFamily:T.sansKr, fontSize:12, color:T.textSub, lineHeight:1.95 }}>{row.desc}</div>
            </div>
          </div>
        ))}
      </div>
...
function CTAPanel() {
  ...
  return (
    <div style={{ background:`radial-gradient(ellipse at 50% 0%,rgba(197,165,114,0.12),transparent 65%),${T.bg2}`, border:`1px solid ${T.goldBorderStrong}`, padding:'36px 32px', display:'flex', flexDirection:'column', justifyContent:'center', position:'relative', overflow:'hidden' }}>
      ...
      <p style={{ fontFamily:T.sans, fontSize:13, color:T.textSub, lineHeight:1.9, marginBottom:28 }}>Founding Season closes when the cohort fills.</p>
      {submitted ? (
        <div style={{ background:T.goldGlow, border:`1px solid ${T.goldBorderStrong}`, padding:'18px 20px' }}>
          ...
          <div style={{ fontFamily:T.sansKr, fontSize:12, color:T.textSub, lineHeight:1.9 }}>론치 시 가장 먼저 알려드립니다.</div>
        </div>
      ) : (
        ...
        <div style={{ maxWidth:1000, margin:'0 auto', display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 60px 1fr 60px 1fr', gap:0, alignItems:'start' }}>
          {[
            ...
            <div key={i} style={{ textAlign:'center', padding:'0 10px' }}>
              ...
              <div style={{ fontFamily:T.sansKr, fontSize:13, color:T.textSub, lineHeight:1.95, maxWidth:200, margin:'0 auto' }}>{s.desc}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding:pad, borderBottom:`1px solid ${T.border}` }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:44 }}>
            ...
            <p style={{ fontFamily:T.sans, fontSize:13, color:T.textSub, maxWidth:480, margin:'0 auto', lineHeight:1.9 }}>티어를 클릭하면 계산기에 자동 반영됩니다.</p>
          </div>
