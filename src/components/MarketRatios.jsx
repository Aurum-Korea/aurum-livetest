...
const CB_HOLDINGS = [
  { country:'미국',      flag:'🇺🇸', tonnes:8133,  pct:68 },
  { country:'독일',      flag:'🇩🇪', tonnes:3352,  pct:68 },
  { country:'이탈리아', flag:'🇮🇹', tonnes:2452,  pct:65 },
  { country:'중국',      flag:'🇨🇳', tonnes:2262,  pct:4  },
  { country:'한국',      flag:'🇰🇷', tonnes:104,   pct:1  },
...
export default function MarketRatios({ prices, krwRate, lang }) {
  ...
  const dow   = 43500; // A4: Dow Jones reference, Q1 2026
  ...
  return (
    <div style={{ background:T.bgCard, border:`1px solid ${T.goldBorder}` }}>
      {/* Tab bar */}
      <div style={{ display:'flex', borderBottom:`1px solid ${T.border}`, overflowX: isMobile ? 'auto' : 'visible' }}>
        {tabs.map((t, i) => (
          <button key={i} onClick={() => setActiveTab(i)} style={{ flex:isMobile?'none':1, whiteSpace:'nowrap', background: activeTab===i ? 'rgba(197,165,114,0.1)' : 'transparent', border:'none', padding: isMobile?'12px 16px':'14px 20px', fontFamily:T.sansKr, fontSize:isMobile?13:14, fontWeight:activeTab===i?700:500, letterSpacing:'0.02em', color:activeTab===i?T.gold:T.textSub, borderBottom:activeTab===i?`2px solid ${T.gold}`:'2px solid transparent', cursor:'pointer', transition:'all 0.2s', marginBottom:-1 }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: isMobile?16:24 }}>
        ...
        {activeTab === 2 && (
          <div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:12 }}>
              {[{label:'Dow (Q1 2026)',val:dow.toLocaleString()},{label:ko?'금 가격':'Gold',val:`$${gold.toFixed(0)}`,gold:true},{label:ko?'비율':'Ratio',val:`${gdRatio}x`,gold:true}].map((s,i)=>(
                <div key={i} style={{ background:'rgba(255,255,255,0.03)', border:`1px solid ${T.border}`, padding:'10px', textAlign:'center' }}>
                  <div style={{ fontFamily:MONO, fontSize:9, color:T.textMuted, marginBottom:3 }}>{s.label}</div>
        ...
        {/* CB Holdings — now first tab (index 0) */}
        {activeTab === 0 && (
          <div>
                ...
                <div key={i} style={{ marginBottom:10, padding: cb.isUser?'10px 12px':'0', background:cb.isUser?'rgba(74,222,128,0.05)':'transparent', border:cb.isUser?'1px solid rgba(74,222,128,0.2)':'none' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:cb.isUser?0:3, alignItems:'center' }}>
                    ...
                    <span style={{ fontFamily:MONO, fontSize:11, color:cb.isUser?'#4ade80':T.gold }}>
                      {cb.isUser ? (ko?'나도 금 적립 시작하기 →':'Start accumulating gold →') : `${cb.tonnes?.toLocaleString()}t`}
                    </span>
                  </div>
                  {!cb.isUser && (
