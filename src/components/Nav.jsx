...
export default function Nav({ page, navigate, lang, setLang, user, setUser, setShowLogin, cart }) {
  ...
  const links = [
    ...
    { page: 'agp-intro',     ko: 'AGP 적금',   en: 'AGP',            isMain: true     },
  ...
  const promos = []; // Promo links removed — campaign pages accessed via homepage CTAs
            ...
            {links.map(l => {
              ...
              if (l.isMain) {
                return (
                  <button key={l.page} onClick={() => navigate(l.page)}
                    className={`founders-btn${isActive ? ' active-founders' : ''}`}
                    style={{ ...btnBase, padding: '0 14px', fontFamily: T.sansKr, fontSize: 13, fontWeight: 600, borderRadius: 0, whiteSpace: 'nowrap' }}>
                    {ko ? l.ko : l.en}
                  </button>
                );
              }
