...
export default function Nav({ page, navigate, lang, setLang, user, setUser, setShowLogin, cart }) {
  ...
  return (
    <nav ref={navRef} style={{
      ...
      {/* Mobile menu — click outside handled by document listener */}
      {isMobile && mobileOpen && (
        <div style={{ background: T.bg1, borderTop: `1px solid ${T.border}`, padding: '8px 16px 20px', display: 'flex', flexDirection: 'column', gap: 2, position:'relative', zIndex:99 }}>
          {/* Founders — plain solid bg, no gradient, no pseudo-elements */}
          <button onClick={() => { navigate('founders'); setMobileOpen(false); }} style={{
            background: '#1a1610',
            border: '1px solid rgba(197,165,114,0.45)',
            borderLeft: '3px solid rgba(197,165,114,0.8)',
            color: '#E3C187',
            padding: '14px 12px', cursor: 'pointer',
            fontFamily: T.serif, fontStyle: 'italic',
            fontSize: 18, textAlign: 'left',
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            marginBottom: 4, outline: 'none', WebkitAppearance: 'none',
            boxSizing: 'border-box',
          }}>
            <span style={{ fontFamily: T.serif, fontStyle:'italic', pointerEvents:'none' }}>Founders Club</span>
