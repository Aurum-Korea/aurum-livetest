...
export default function DashboardPreview() {
  // Plan toggle drives monthly contribution
  const [plan, setPlan] = useState('starter');     // starter → ₩500K · power → ₩2M
  // Single input: months of accumulation so far (0-36)
  const [months, setMonths] = useState(14);

  const monthly = plan === 'starter' ? 500_000 : 2_000_000;

  // Trajectory computed to the ceiling (36mo) so the sparkline always shows the full arc
  const trajectory = useMemo(() => buildTrajectory(monthly, 36), [monthly]);
  // Active row · at current slider position
  ...
  // Sparkline · ONLY the trajectory UP TO the slider position · shows the "path so far"
  const sparkPath = trajectory.slice(0, Math.max(1, months));

  // Year-label of the slider ("Y1 · M2")
  const yrLbl = months === 0 ? 'day 1' :
                months < 12 ? `M${months}` :
                `Y${Math.floor((months - 1) / 12) + 1} · M${((months - 1) % 12) + 1}`;

  // Displayed delta · vs month 1 of the trajectory
  const delta = sparkPath.length > 1
    ? ((sparkPath[sparkPath.length - 1].krwValue - sparkPath[0].krwValue) / sparkPath[0].krwValue) * 100
    : 0;
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18, gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {[
            { k: 'starter', lbl: '₩500K / mo' },
            { k: 'power',   lbl: '₩2M / mo' },
          ].map(v => (
            <button key={v.k} onClick={() => setPlan(v.k)} style={{
              padding: '5px 12px',
              border: `1px solid ${plan === v.k ? T.gold : T.border}`,
              background: plan === v.k ? T.goldGlow : 'transparent',
              color: plan === v.k ? T.gold : T.sub,
