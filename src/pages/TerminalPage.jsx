      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 16px', height: 40, flexWrap: 'wrap', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{
              width: 22, height: 22, border: `1px solid ${T.goldBorder}`,
              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
              fontFamily: T.serif, fontSize: 11, color: T.gold, fontWeight: 500, letterSpacing: '0.03em',
            }}>AU</span>
            <span style={{ fontFamily: T.serif, fontStyle: 'italic', fontSize: 15, color: T.text }}>Aurum</span>
            <span style={{ fontSize: 8, color: T.goldD, letterSpacing: '0.22em', marginLeft: 2 }}>TERMINAL</span>
          </div>
          <div style={{ width: 1, height: 14, background: T.border }} />
          {['HOME', 'POS', 'ORDER', 'VAULT', 'HIST', 'ACCT'].map((k, i) => (
            <button key={i} style={{
              ...
              letterSpacing: '0.18em', fontFamily: T.mono,
            }}>{k}</button>
          ))}
        </div>
...
// ═══════════════════════════════════════════════════════════════════════════
function Panel({ num, title, sub, live = false, badge = false, children, compact = false }) {
  return (
...
function PositionOverview() {
  ...
  return (
    <Panel num="01" title="포지션 · Position" sub="gold · allocated · Malca-Amit SGP" live>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 0, background: T.bg3, border: `1px solid ${T.border}`, borderTop: 'none', padding: '14px 20px', marginTop: 0 }} className="next-strip">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button style={{ background: 'transparent', border: `1px solid ${T.goldBorder}`, color: T.gold, padding: '8px 14px', fontFamily: T.mono, fontSize: 10, letterSpacing: '0.18em' }}>SKIP ONCE</button>
            <button style={{ background: 'transparent', border: `1px solid ${T.goldBorder}`, color: T.gold, padding: '8px 14px', fontFamily: T.mono, fontSize: 10, letterSpacing: '0.18em' }}>MODIFY</button>
          </div>
        </div>
      </div>
    </Panel>
  ...
  return (
    <Panel num="02" title="가격 비교 · Korean arb" sub="live · 1g" live>
      ...
      <div style={{ marginTop: 12, padding: '10px 14px', background: T.deepBlack, border: `1px dashed ${T.border}`, fontFamily: T.mono, fontSize: 10, color: T.muted, lineHeight: 1.6, letterSpacing: '0.06em' }}>
        한국 소매 대비 귀하의 1g 절감: <span style={{ color: T.green, fontWeight: 600 }}>₩{Math.round((per1g_kr_retail - per1g_aurum_founders) * 1440.20).toLocaleString()}</span> · 다음 debit(₩1M)에서 약 <span style={{ color: T.green, fontWeight: 600 }}>₩{Math.round((per1g_kr_retail - per1g_aurum_founders) * 1440.20 * (1_000_000 / (per1g_aurum_founders * 1440.20))).toLocaleString()}</span> 절감 예상.
      </div>
    </Panel>
  );
}
...
function OrderTicket() {
  ...
  return (
    <Panel num="03" title="주문 수정 · Modify subscription" sub="slider → live tier reassign">
      <div style={{ background: T.card, border: `1px solid ${T.goldBorder}`, display: 'grid', gridTemplateColumns: '1fr', gap: 0 }} className="order-grid">
        ...
        <div style={{ padding: 24, background: 'linear-gradient(180deg, #0a0906, #060504)' }}>
          ...
          <button style={{ width: '100%', background: T.gold, color: T.bg, padding: 14, fontFamily: T.mono, fontWeight: 700, fontSize: 11, letterSpacing: '0.22em', cursor: 'pointer' }}>
            ▶ APPLY CHANGES
          </button>
...
function VaultStatus() {
  ...
  return (
    <Panel num="04" title="볼트 상태 · Vault" sub="Malca-Amit FTZ · Changi · live">
...
function EventLog() {
  return (
    <Panel num="05" title="이벤트 로그 · Event stream" badge>
...
function HoldingsLedger() {
  const bars = [
    { sn: 'AU-KR-000421', wt: '10.00g', pure: '999.9', buy: '2025-11-15', cost: '₩682,400', bar: 'PAMP 10g' },
    { sn: 'AU-KR-000498', wt: '10.00g', pure: '999.9', buy: '2025-12-15', cost: '₩694,200', bar: 'PAMP 10g' },
    { sn: 'AU-KR-000562', wt: '5.00g',  pure: '999.9', buy: '2026-01-15', cost: '₩351,800', bar: 'Argor 5g' },
    { sn: 'AU-KR-000634', wt: '5.00g',  pure: '999.9', buy: '2026-02-15', cost: '₩358,100', bar: 'Argor 5g' },
    { sn: 'AU-KR-000716', wt: '6.22g',  pure: '999.9', buy: '2026-03-15', cost: '₩446,900', bar: 'Aggregated' },
    { sn: 'AU-KR-000802', wt: '6.15g',  pure: '999.9', buy: '2026-04-15', cost: '₩449,300', bar: 'Aggregated' },
  ];
  return (
...
function AccountStrip() {
  return (
    <Panel num="08" title="계정 · Account" compact>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 1, background: T.border }}>
        {[
