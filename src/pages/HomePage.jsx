    ...
    <div style={{ background:'linear-gradient(150deg,#08070a,#0d0c12)', borderBottom:'1px solid rgba(197,165,114,0.12)', position:'relative', overflow:'hidden', minHeight:isMobile?'auto':480 }}>
      ...
      <div className="aurum-container" style={{ position:'relative', zIndex:1, paddingTop:isMobile?32:80, paddingBottom:isMobile?32:80, display:isMobile?'block':'flex', gap:56, alignItems:'center' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <p style={{ fontFamily:SANS, fontSize:15, color:'#a09080', lineHeight:1.9, marginBottom:28 }}>
            {ko ? '총 구매금액이 5개 단계를 통과할 때마다, 그 할인은 평생 유지됩니다.' : 'Each time your total purchases pass one of 5 thresholds, that discount stays with you for life.'}
          </p>
          <div style={{ marginBottom:32 }}>
            {gates.map((g, i) => (
...
function AGPAccumulatorCalc({ prices, krwRate, lang, tiers }) {
  ...
  return (
    <div style={{ background:'rgba(14,12,8,0.8)', border:'1px solid rgba(197,165,114,0.15)', padding:isMobile?'18px 14px':'24px 24px', marginTop:20 }}>
      ...
      <div style={{ fontFamily:MONO, fontSize:20, color:'#f5f0e8', fontWeight:700, marginBottom:8 }}>
        ₩{Math.round(monthly/10000).toLocaleString()}{ko ? '만원/월' : '/mo'}
      </div>
      <input type="range" min="200000" max="5000000" step="100000" value={monthly}
        style={{ width:'100%', marginBottom:16 }} onChange={e => setMonthly(+e.target.value)} />
      <div style={{ display:'flex', gap:3, alignItems:'flex-end', height:72, marginBottom:6 }}>
        {Array.from({ length:12 }, (_, i) => (
          <div key={i} style={{ flex:1, height:`${Math.round(((i+1)/12)*60)}px`, background:i===11?'#c5a572':'rgba(197,165,114,0.22)' }}>
            {i===11 && <div style={{ position:'relative', bottom:'100%', fontFamily:MONO, fontSize:9, color:'#c5a572', textAlign:'center' }}>{totalGrams.toFixed(2)}g</div>}
          </div>
        ))}
      </div>
...
function FoundersMiniCalc({ krwRate, lang, navigate }) {
  ...
  return (
    <div style={{ background:'rgba(197,165,114,0.04)', border:'1px solid rgba(197,165,114,0.15)', padding:'20px 20px', marginTop:20 }}>
      ...
      <div style={{ fontFamily:MONO, fontSize:18, color:'#f5f0e8', marginBottom:8 }}>
        ₩{Math.round(monthly/10000).toLocaleString()}{ko ? '만원/월' : '/mo'}
      </div>
      <input type="range" min="200000" max="5000000" step="100000" value={monthly}
        style={{ width:'100%', marginBottom:16 }} onChange={e => setMonthly(+e.target.value)} />
...
function HowItWorks({ lang, isMobile }) {
  ...
  const steps = [
    { n:'STEP 01', t:ko?'신원 확인':'Verify Identity',
      d:ko?'NICE·KCB 본인인증 (1분). 국제 AML 기준 KYC.':'NICE/KCB identity check (1 min). AML KYC.' },
...
function GoldKRWIndexedChart({ lang, isMobile }) {
  ...
  return (
    <div ref={ref} style={{ opacity:vis?1:0, transition:'opacity 0.8s ease', marginBottom:24 }}>
      <div style={{ ...eyebrowStyle, marginBottom:8 }}>{ko ? '2020년 1월 = 100 인덱스' : 'Indexed: Jan 2020 = 100'}</div>
      <canvas ref={canvasRef} width={isMobile?320:480} height={200} style={{ display:'block', width:'100%', maxWidth:isMobile?320:480 }} />
    </div>
  );
}
...
function SavingsComparisonChart({ prices, krwRate, lang, isMobile }) {
  ...
  return (
    <div ref={ref} style={{ opacity:vis?1:0, transition:'opacity 0.8s ease' }}>
      <div style={{ fontFamily:MONO, fontSize:10, color:T.goldDim, letterSpacing:'0.16em', textTransform:'uppercase', marginBottom:8, display:'flex', justifyContent:'space-between' }}>
      ...
      <input type="range" min="50000" max="1000000" step="50000" value={monthly}
        style={{ width:'100%', marginBottom:8 }} onChange={e => setMonthly(+e.target.value)} />
      <canvas ref={canvasRef} width={isMobile?300:420} height={180} style={{ display:'block', width:'100%', maxWidth:isMobile?300:420 }} />
    </div>
  );
}
...
export default function HomePage({ navigate, prices, krwRate, currency, setCurrency, lang }) {
  ...
  const whyItems = [
    ...
    { icon:<IconBox><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/><line x1="2" y1="20" x2="22" y2="20" strokeDasharray="2 2"/></svg></IconBox>, title: lang==='ko'?'국제 현물가 직거래 — 김치 프리미엄 없음':'International Spot Price — No Kimchi Premium', content: lang==='ko'?'시중 금통장, 국내 귀금속 딜러 — 모두 국제 현물가 대비 유의미한 프리미엄이 구조적으로 포함되어 있습니다. Aurum은 LBMA 국제 현물가 기준의 투명한 수수료만 적용합니다.':'Korea retail channels carry a structural premium above international spot due to 10% VAT. Aurum operates at LBMA international spot with a single transparent fee.' },
    ...
    <div>
        ...
        <div className="aurum-container" style={{ position:'relative', zIndex:1, display:isMobile?'block':'flex', alignItems:'center', gap:isMobile?0:80, paddingTop:isMobile?32:96, paddingBottom:isMobile?40:96, width:'100%' }}>
          ...
          <div style={{ flex:1, maxWidth:isMobile?'100%':520 }}>
            ...
            <h1 style={{ fontFamily:lang==='ko'?T.serifKrDisplay:SERIF, fontSize:isMobile?32:54, fontWeight:300, color:'#f5f0e8', lineHeight:1.1, margin:'0 0 20px' }}>
              {lang==='ko'
                ?(<><span style={{ color:'#c5a572', fontWeight:400, fontStyle:'italic', fontFamily:T.serif }}>금을 가진 사람들의</span><br />플랫폼.</>)
            ...
            <p style={{ fontFamily:SANS, fontSize:isMobile?14:16, color:'#a09080', lineHeight:1.95, margin:'0 0 10px' }}>
              {lang==='ko'
                ?'ETF도, 금통장도 아닙니다. 실물 금이 싱가포르 금고에 — 고객님의 이름으로. 국내 김치 프리미엄 없이.'
        ...
        <div className="aurum-container" style={{ paddingTop: isMobile ? 32 : 64, paddingBottom: isMobile ? 32 : 64 }}>
          <div style={{ textAlign:'center', marginBottom: isMobile?20:36 }}>
            ...
            <h2 style={{ fontFamily:SERIF, fontStyle:'italic', fontSize: isMobile?22:32, color:'#f5f0e8', fontWeight:300, margin:'8px 0 0' }}>
              {lang==='ko'?<>숫자가 말합니다 — <span style={{ color:'#c5a572' }}>지금이 맞습니다</span></> : <>The data says — <span style={{ color:'#c5a572' }}>now is the time</span></>}
            </h2>
          </div>
      ...
      {(() => {
        ...
        const pvpRows = [
          ...
          { label: lang==='ko'?'관할권 분산':'Jurisdiction',            bad: lang==='ko'?'국내 only':'Korea only',                    good: lang==='ko'?'싱가포르 + 한국 이중':'Singapore + Korea Dual' },
        ];
        return (
      ...
      {(() => {
        ...
        <div className="aurum-container" style={{ paddingTop: isMobile ? 40 : 72, paddingBottom: isMobile ? 32 : 60 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
            <div>
              <span style={eyebrowStyle}>{lang === 'ko' ? '지금도 내고 있습니다' : "You're Paying Right Now"}</span>
              <h2 style={{ fontFamily: SERIF, fontSize: isMobile ? 28 : 38, color: '#f5f0e8', fontWeight: isMobile ? 300 : 400, margin: 0 }}>{lang === 'ko' ? '지금 이 순간에도 내고 있습니다.' : 'Every second, you\'re overpaying.'}</h2>
