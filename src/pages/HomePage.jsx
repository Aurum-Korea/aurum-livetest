        ...
        <div className="aurum-container" style={{ position:'relative', zIndex:1, display:isMobile?'block':'flex', alignItems:'center', gap:48, paddingTop:isMobile?28:80, paddingBottom:isMobile?32:80, width:'100%' }}>
          ...
          <div style={{ flex:1, maxWidth:isMobile?'100%':520 }}>
            ...
            {prices?.gold && krwRate ? (() => {
              ...
              const pct = '20';
              return (
                <div style={{ display:'inline-flex', alignItems:'center', gap:10, background:'rgba(248,113,113,0.07)', border:'1px solid rgba(248,113,113,0.2)', padding:'6px 14px', marginBottom:20 }}>
                  <span style={{ width:6, height:6, borderRadius:'50%', background:'#f87171', flexShrink:0, animation:'pulse 1.6s ease-in-out infinite' }} />
                  <span style={{ fontFamily:MONO, fontSize:10, color:'#f87171', letterSpacing:'0.12em' }}>
                    {lang==='ko' ? `김치 프리미엄 지금: +${pct}% · ₩${gap.toLocaleString('ko-KR')}/oz` : `Kimchi Premium now: +${pct}% · ₩${gap.toLocaleString('ko-KR')}/oz`}
                  </span>
                </div>
              );
            })() : null}
            ...
            <div style={{ marginTop:16, display:'flex', gap:12, flexWrap:'wrap', justifyContent:isMobile?'center':'flex-start' }}>
              {(lang==='ko'
                ?["Lloyd's 보험",'LBMA 승인','완전 배분 보관','매일 공개 감사','🇸🇬 싱가포르 MAS']
                :["Lloyd's Insured",'LBMA Approved','Fully Allocated','Daily Audit','🇸🇬 Singapore MAS']
              ).map((t,i)=>(
