  ...
  return (
    <div style={{ position:'relative', width:300, height:420 }}>
      ...
      <div style={{ position:'absolute', top:16, left:'50%', transform:'translateX(-50%)', width:176, height:112, background:'linear-gradient(135deg,#2a2418 0%,#4a3e26 35%,#C5A572 50%,#E3C187 55%,#C5A572 62%,#6a5a3a 80%,#2a2418 100%)', boxShadow:'0 24px 50px rgba(197,165,114,0.22), 0 0 0 1px rgba(197,165,114,0.4), inset 0 1px 0 rgba(255,255,255,0.12)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', animation:'ingot-float 4s ease-in-out infinite' }}>
        ...
        <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:11, fontWeight:700, color:'rgba(26,24,20,0.85)', letterSpacing:'0.08em' }}>› GP▌</div>
...
function AGPLaunchPane({ lang, navigate, prices, krwRate }) {
  ...
  return (
    <div style={{ background:'linear-gradient(150deg,#0d0b07,#161208)', borderTop:'1px solid rgba(197,165,114,0.12)', borderBottom:'1px solid rgba(197,165,114,0.12)', position:'relative', overflow:'hidden', minHeight:isMobile?'auto':480 }}>
      ...
      <div className="aurum-container" style={{ position:'relative', zIndex:1, paddingTop:isMobile?56:96, paddingBottom:isMobile?56:96, display:isMobile?'block':'flex', gap:60, alignItems:'center' }}>
        {/* Left — text + tiers */}
        <div style={{ flex:'0 0 auto', maxWidth:isMobile?'100%':500, marginBottom:isMobile?40:0 }}>
          {/* Mobile brand mark */}
          {isMobile && (
            <div style={{ marginBottom:20 }}>
              <svg viewBox="0 0 240 70" width={Math.round(240*0.6)} height={Math.round(70*0.6)} style={{ display:'block' }}>
                <defs><style>{`@keyframes gp-mb{0%,100%{opacity:1}50%{opacity:0}}.gp-mb-c{animation:gp-mb 1.2s step-end infinite}`}</style></defs>
                <text x="8" y="22" fontFamily="'JetBrains Mono','Courier New',monospace" fontSize="16" fill="#f5f0e8">₩</text>
                <rect x="28" y="16" width="7" height="6" fill="#1e1408"/><rect x="37" y="13" width="7" height="9" fill="#5a3c18"/>
                <rect x="46" y="9" width="7" height="13" fill="#C5A572"/><rect x="55" y="5" width="7" height="17" fill="#E3C187"/>
                ...
                <text x="87" y="22"><tspan fontFamily="'Cormorant Garamond',serif" fontStyle="italic" fontSize="14" fill="#E3C187">Au</tspan><tspan fontFamily="'JetBrains Mono',monospace" fontSize="10" fill="#7a6d58" dx="3">(금)</tspan></text>
                <line x1="8" y1="28" x2="232" y2="28" stroke="#1e1c10" strokeWidth=".4"/>
                <text x="8" y="46" fontFamily="'JetBrains Mono',monospace" fontSize="19" fill="#C5A572">›</text>
                <text x="22" y="46" fontFamily="'JetBrains Mono',monospace" fontSize="19" fill="#f0ece4">GoldPath</text>
                <rect x="118" y="30" width="2" height="16" fill="#C5A572" className="gp-mb-c"/>
                <text x="8" y="62" fontFamily="'JetBrains Mono',monospace" fontSize="14" fill="#7a6d58" letterSpacing=".2em">금환</text>
              </svg>
            </div>
          )}
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:18 }}>
            ...
            <span style={{ fontFamily:MONO, fontSize:11, color:'#c5a572', letterSpacing:'0.22em', textTransform:'uppercase' }}>
              {ko ? 'GoldPath · 금환 · 원화를 금으로' : 'GoldPath · KRW to Gold · Automated'}
            </span>
          </div>
          ...
          <p style={{ fontFamily:SANS, fontSize:16, color:'#f0ece4', lineHeight:1.85, marginBottom:12 }}>
            {ko ? <>가입 첫 달, 선택한 등급에 따라 <strong style={{ color:'#4ade80' }}>최대 ₩5,000,000 상당의 금이 즉시 적립</strong>됩니다.</> : <>In your first month, receive <strong style={{ color:'#4ade80' }}>up to ₩5,000,000 in gold credited instantly</strong> — based on your tier.</>}
          </p>
          <div style={{ display:'flex', flexDirection:'column', gap:6, marginBottom:28 }}>
            {[
              ko ? '국제 현물가 + 2% 투명 프리미엄 — 숨겨진 수수료 없음' : 'International spot + 2% transparent premium — no hidden fees',
              ko ? '100g 달성 시 LBMA 승인 실물 바로 무료 전환' : 'Free conversion to LBMA bar when you hit 100g',
              ko ? '싱가포르 금고에, 귀하의 이름으로. 언제든 해지 가능.' : 'Singapore vault, in your name. Cancel anytime.',
            ].map((b, i) => (
              <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:10 }}>
                <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:11, color:'#c5a572', flexShrink:0, marginTop:2 }}>›</span>
                <span style={{ fontFamily:SANS, fontSize:14, color:'#a09080', lineHeight:1.7 }}>{b}</span>
              </div>
            ))}
          </div>
          ...
          <button onClick={() => navigate('agp-intro')}
            style={{ background:'#c5a572', border:'none', color:'#0a0a0a', padding: '14px 28px', fontFamily:SANS, fontSize:14, fontWeight:700, cursor:'pointer', letterSpacing:'0.02em', transition:'all 0.2s' }}
        ...
        {/* Right — GoldPath brand moment (GoldPath·2 C) */}
        {!isMobile && (
          <div style={{ flex:1, display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', gap:32 }}>
            <svg viewBox="0 0 240 70" style={{ display:'block', width:'100%', maxWidth:580, height:'auto' }}>
              <defs><style>{`@keyframes hp-blink{0%,100%{opacity:1}50%{opacity:0}}.hp-cur{animation:hp-blink 1.2s step-end infinite}`}</style></defs>
              <text x="8" y="22" fontFamily="'JetBrains Mono','Courier New',monospace" fontSize="16" fill="#f5f0e8">₩</text>
              ...
              <rect x="118" y="30" width="2" height="16" fill="#C5A572" className="hp-cur"/>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10, width:'100%', maxWidth:280 }}>
              {[
                { v:'₩200K+', l:ko?'월 최소 적립':'Min. monthly' },
                { v:'100g',   l:ko?'실물 바 전환 기준':'Physical bar at' },
                { v:'+2%',    l:ko?'LBMA 현물가 대비 투명 프리미엄':'Transparent premium vs LBMA' },
              ].map((s,i) => (
                <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 14px', background:'rgba(255,255,255,0.02)', borderBottom:'1px solid rgba(197,165,114,0.06)' }}>
                  <span style={{ fontFamily:SANS, fontSize:13, color:'#7a7060' }}>{s.l}</span>
                  <span style={{ fontFamily:MONO, fontSize:15, color:'#C5A572', fontWeight:700 }}>{s.v}</span>
                </div>
              ))}
            </div>
          </div>
...
function FoundersClubPane({ lang, navigate, krwRate }) {
  ...
  return (
    <div style={{ background:'linear-gradient(150deg,#08070a,#0d0c12)', borderBottom:'1px solid rgba(197,165,114,0.12)', position:'relative', overflow:'hidden', minHeight:isMobile?'auto':480 }}>
      ...
      <div className="aurum-container" style={{ position:'relative', zIndex:1, paddingTop:isMobile?56:96, paddingBottom:isMobile?56:96, display:isMobile?'block':'flex', gap:56, alignItems:'center' }}>
        {/* Left — gate visual */}
        {!isMobile && (
        ...
        <div style={{ flex:1 }}>
          ...
          <p style={{ fontFamily:SANS, fontSize:16, color:'#a09080', lineHeight:1.9, marginBottom:28 }}>
          ...
          <button onClick={() => navigate('founders')}
            style={{ background:'transparent', border:'1px solid rgba(197,165,114,0.5)', color:'#c5a572', padding: '14px 28px', fontFamily:SANS, fontSize:14, fontWeight:600, cursor:'pointer', transition:'all 0.2s' }}
        ...
        {tiers.map((t, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              border: `1px solid ${isActive ? 'rgba(45,212,191,0.45)' : 'rgba(197,165,114,0.06)'}`,
              transition:'all 0.25s',
            }}>
              <div style={{ width:8, height:8, borderRadius:'50%', background: isActive ? '#2dd4bf' : '#333', flexShrink:0, transition:'background 0.25s' }} />
              <span style={{ fontFamily:SANS, fontSize:11, color: isActive ? '#f5f0e8' : '#555', flex:1, transition:'color 0.25s' }}>{ko ? t.name : t.nameEn}</span>
              <span style={{ fontFamily:MONO, fontSize:10, color: isActive ? 'rgba(45,212,191,0.6)' : '#555' }}>{t.min}</span>
              <span style={{ fontFamily:MONO, fontSize:11, color: isActive ? '#2dd4bf' : '#555', fontWeight: isActive ? 700 : 400 }}>{t.gift}</span>
            </div>
          );
        })}
      </div>
  ...
  return (
    <div ref={ref} style={{
      ...
      <div className="aurum-container" style={{ paddingTop: isMobile ? 40 : 64, paddingBottom: isMobile ? 40 : 64 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: isMobile ? 24 : 36, flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom: 8 }}>
              <span style={{ fontFamily:MONO, fontSize:22, color:'rgba(197,165,114,0.12)', fontWeight:700, lineHeight:1, letterSpacing:'-0.02em', flexShrink:0 }}>02</span>
              <div style={{ width:1, height:20, background:'rgba(197,165,114,0.15)', flexShrink:0 }} />
              <div style={{ fontFamily: MONO, fontSize: 10, color: 'rgba(197,165,114,0.5)', letterSpacing: '0.22em', textTransform: 'uppercase' }}>
        ...
        {/* Bridge 02 → 03 */}
        <div style={{ display:'flex', justifyContent:'flex-end', alignItems:'center', gap:10, marginTop:20, paddingTop:14, borderTop:'1px solid rgba(197,165,114,0.06)' }}>
          <span style={{ fontFamily:MONO, fontSize:9, color:'rgba(197,165,114,0.3)', letterSpacing:'0.1em' }}>
            {ko ? '그렇다면 한국 가계는 어디에 있는가?' : 'Where do Korean households stand?'}
          </span>
    ...
    <div ref={ref} style={{ borderBottom: '1px solid rgba(197,165,114,0.08)', background: '#0d0b08', opacity: vis?1:0, transition: 'opacity 0.8s ease' }}>
      <div className="aurum-container" style={{ paddingTop: isMobile?32:64, paddingBottom: isMobile?32:64 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:16, marginBottom:24 }}>
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
              <span style={{ fontFamily:MONO, fontSize:22, color:'rgba(197,165,114,0.12)', fontWeight:700, lineHeight:1, letterSpacing:'-0.02em', flexShrink:0 }}>01</span>
              <div style={{ width:1, height:20, background:'rgba(197,165,114,0.15)', flexShrink:0 }} />
              <div style={{ fontFamily:MONO, fontSize:10, color:'rgba(74,222,128,0.6)', letterSpacing:'0.22em', textTransform:'uppercase' }}>
        ...
        <p style={{ fontFamily:SANS, fontSize:14, color:'#555', marginTop:10, lineHeight:1.7 }}>
        ...
        <div style={{ display:'flex', justifyContent:'flex-end', alignItems:'center', gap:10, marginTop:20, paddingTop:14, borderTop:'1px solid rgba(197,165,114,0.06)' }}>
          <span style={{ fontFamily:MONO, fontSize:9, color:'rgba(197,165,114,0.3)', letterSpacing:'0.1em' }}>
            {ko ? '이것이 원화에 무엇을 의미하는가?' : 'What does this mean for KRW?'}
          </span>
    ...
    <div ref={ref} style={{ background:'#0a0a0a', borderBottom:'1px solid rgba(197,165,114,0.08)', opacity:vis?1:0, transform:vis?'translateY(0)':'translateY(16px)', transition:'opacity 0.7s ease, transform 0.7s ease' }}>
      <div className="aurum-container" style={{ paddingTop: isMobile?40:64, paddingBottom: isMobile?40:64 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
          <span style={{ fontFamily:MONO, fontSize:22, color:'rgba(197,165,114,0.12)', fontWeight:700, lineHeight:1, letterSpacing:'-0.02em', flexShrink:0 }}>03</span>
          <div style={{ width:1, height:20, background:'rgba(197,165,114,0.15)', flexShrink:0 }} />
          <div style={{ fontFamily:MONO, fontSize:10, color:'rgba(197,165,114,0.5)', letterSpacing:'0.22em', textTransform:'uppercase' }}>
        ...
        <p style={{ fontFamily:SANS, fontSize:14, color:'#555', marginBottom: isMobile?24:36, maxWidth:600 }}>
          {ko ? '0.3% vs 20% — 이 격차는 구조적입니다. 정보의 부재가 아니라, 접근 방식의 부재입니다.' : '0.3% vs 20% — this gap is structural. Not a lack of information, but a lack of access.'}
        </p>
        ...
        <div style={{ display:'flex', justifyContent:'flex-end', alignItems:'center', gap:10, marginTop:20, paddingTop:14, borderTop:'1px solid rgba(197,165,114,0.06)' }}>
          <span style={{ fontFamily:MONO, fontSize:9, color:'rgba(197,165,114,0.3)', letterSpacing:'0.1em' }}>
            {ko ? '그러나 — 어떤 금을 어떻게 보유해야 하는가?' : 'But — which gold, and how?'}
          </span>
          <span style={{ fontFamily:MONO, fontSize:9, color:'rgba(197,165,114,0.4)', letterSpacing:'0.1em' }}>↓</span>
        </div>
      </div>
    </div>
  );
...
// ─── SavingsSection ───────────────────────────────────────────────────────────
// ─── Animated savings % counter (Math·2 D) ───────────────────────────────────
function SavingsPctCounter({ pct, label, vis, isMobile }) {
  const [count, setCount] = useState(0);
  const triggered = useRef(false);
  useEffect(() => {
    if (!vis || triggered.current) return;
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap: isMobile ? 12 : 20, padding:'14px 0' }}>
      <div style={{ lineHeight:1 }}>
        <div style={{ fontFamily:MONO, fontSize: isMobile ? 42 : 56, color:'#4ade80', fontWeight:700, letterSpacing:'-0.02em', lineHeight:1 }}>
          −{count}%
        </div>
        <div style={{ fontFamily:MONO, fontSize:9, color:'rgba(74,222,128,0.5)', letterSpacing:'0.16em', textTransform:'uppercase', marginTop:6 }}>
          {label}
        </div>
      </div>
      <div style={{ flex:1, height:1, background:'linear-gradient(90deg,rgba(74,222,128,0.3),transparent)' }} />
    </div>
  );
}
        ...
        {[
          ...
          <div key={i} className="magnetic-card" style={{ background: '#141414', border: '1px solid rgba(197,165,114,0.20)', padding: '22px 22px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ margin: '16px 0 12px' }}>
              <SavingsPctCounter pct={c.krPrem - c.auPrem} label={lang==='ko' ? 'Aurum vs 국내 소매 절감율' : 'Aurum vs Korea Retail savings'} vis={vis} isMobile={isMobile} />
            </div>
...
function WhyAurumSection({ lang, isMobile }) {
  ...
  const whyItems = [
    ...
    { icon:<IconBox><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="17" x2="11" y2="17"/><polyline points="11 13 12.5 15 16 11"/></svg></IconBox>, title: lang==='ko'?'매일 공개 감사 · 100% 실물 백킹':'Daily Public Audit · 100% Physical Backing', content: lang==='ko'?'모든 GoldPath 그램 및 배분 보관 금속의 100% 실물 백킹을 매일 감사 리포트로 공개합니다. 레버리지, 재담보, 파생상품 노출 일체 없습니다. 귀하의 금은 귀하의 금입니다.':'100% physical backing for every GoldPath gram and allocated metal is published daily in the audit report. No leverage, no rehypothecation, no derivatives exposure. Your gold is your gold.' },
  ];
  return (
...
function FinalCTASection({ lang, isMobile, goldKR, goldAU, navigate }) {
  ...
  return (
    <div ref={ref} style={{
      ...
      <div className="aurum-container" style={{ paddingTop: isMobile ? 56 : 96, paddingBottom: isMobile ? 56 : 96, textAlign: 'center' }}>
        <div style={{ fontFamily: MONO, fontSize: 9, color: 'rgba(197,165,114,0.5)', letterSpacing: '0.24em', textTransform: 'uppercase', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'center' }}>
          <button onClick={() => navigate('campaign-agp-launch')} style={{ background: '#c5a572', border: 'none', color: '#0a0a0a', padding: isMobile ? '15px 24px' : '16px 40px', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: SANS, width: isMobile ? '100%' : 'auto' }}>
            {lang === 'ko' ? '금환 GoldPath 시작하기 →' : 'Start GoldPath 금환 →'}
          </button>
          <button onClick={() => navigate('campaign-founders')} style={{ background: 'transparent', border: '1px solid rgba(197,165,114,0.5)', color: '#c5a572', padding: isMobile ? '15px 24px' : '16px 40px', fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: SANS, width: isMobile ? '100%' : 'auto' }}>
...
export default function HomePage({ navigate, prices, krwRate, currency, setCurrency, lang }) {
  const isMobile = useIsMobile();
  const fP = usd => currency === 'KRW' ? fKRW(usd * krwRate) : fUSD(usd);
  // Layout·1 A: two-tier padding standard
  const SP = { paddingTop: isMobile ? 40 : 64,  paddingBottom: isMobile ? 40 : 64  }; // standard sections
  const FP = { paddingTop: isMobile ? 56 : 96,  paddingBottom: isMobile ? 56 : 96  }; // feature sections
  ...
  return (
    <div>
        ...
        <div className="aurum-container" style={{ position:'relative', zIndex:1, display:isMobile?'block':'flex', alignItems:'center', gap:isMobile?0:80, paddingTop:isMobile?32:96, paddingBottom:isMobile?40:96, width:'100%' }}>
          ...
          <div style={{ flex:1, maxWidth:isMobile?'100%':520 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:20, flexWrap:'nowrap', overflow:'hidden' }}>
              ...
              <span style={{ fontFamily:MONO, fontSize:isMobile?9:10, color:'rgba(197,165,114,0.6)', letterSpacing:'0.2em', textTransform:'uppercase', whiteSpace:'nowrap' }}>
                {lang==='ko'?'중앙은행이 매입하는 자산 · 원화로 · 자동으로':'The asset central banks are buying · In KRW · Automated'}
              </span>
              {!isMobile && <div style={{ width:28, height:1, background:'#c5a572', flexShrink:0 }} />}
            </div>
            <h1 style={{ fontFamily:lang==='ko'?T.serifKrDisplay:SERIF, fontSize:isMobile?'clamp(32px,8vw,44px)':'clamp(36px,5vw,60px)', fontWeight:lang==='ko'?500:300, color:'#f5f0e8', lineHeight:1.08, margin:'0 0 20px' }}>
              {lang==='ko'
                ?(<>원화를 금으로<br /><span style={{ color:'#c5a572', fontFamily:T.serif, fontStyle:'italic', fontWeight:300 }}>전환하십시오.</span></>)
                :(<><span style={{ color:'#c5a572', fontStyle:'italic' }}>Convert KRW</span><br />to gold.</>)}
            </h1>
              ...
              return (
                <div style={{ display:'flex', alignItems:'center', gap:10, background:'rgba(248,113,113,0.1)', border:'1px solid rgba(248,113,113,0.35)', padding:'9px 16px', marginBottom:20, width:'fit-content' }}>
                  ...
                  <span style={{ fontFamily:MONO, fontSize:isMobile?12:13, color:'#f87171', letterSpacing:'0.08em', fontWeight:600, animation:'kimchiBlink 2s ease-in-out infinite' }}>
                    {lang==='ko' ? `김치 프리미엄 지금: +${kimchiPct}% · ₩${gap.toLocaleString('ko-KR')}/oz` : `Kimchi Premium now: +${kimchiPct}% · ₩${gap.toLocaleString('ko-KR')}/oz`}
                  </span>
                </div>
              );
            })() : null}
            <div style={{ display:'flex', gap:12, flexDirection:isMobile?'column':'row' }}>
              <button onClick={() => navigate('shop-physical')} style={{ flex:1, background:'linear-gradient(135deg,#c5a572,#8a6914)', color:'#fff', border:'none', padding: isMobile?'15px 20px':'16px 40px', fontSize:15, fontFamily:SANS, fontWeight:700, borderRadius:0, cursor:'pointer' }}>
      ...
      {/* ── Thesis spine — 01 / 02 / 03 with running sidebar ── */}
      <div id="macro-thesis" style={{ position:'relative' }}>
        {!isMobile && (
          <div style={{ position:'absolute', left:0, top:0, bottom:0, width:28, display:'flex', alignItems:'center', justifyContent:'center', zIndex:2, pointerEvents:'none' }}>
            <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, color:'rgba(197,165,114,0.18)', letterSpacing:'0.35em', textTransform:'uppercase', transform:'rotate(-90deg)', whiteSpace:'nowrap', userSelect:'none' }}>왜 지금인가</span>
          </div>
        )}
