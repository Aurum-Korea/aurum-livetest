        ...
        <div style={{ flex:'0 0 auto', maxWidth:isMobile?'100%':500, marginBottom:isMobile?40:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:18 }}>
            ...
            <span style={{ fontFamily:MONO, fontSize:11, color:'#c5a572', letterSpacing:'0.22em', textTransform:'uppercase' }}>
              {ko ? 'AGP 자동 적금 · 지금 가입 시 특별 혜택' : 'AGP Launch Event'}
            </span>
          </div>
          ...
          <p style={{ fontFamily:SANS, fontSize:15, color:'#a09080', lineHeight:1.9, marginBottom:28 }}>
            {ko ? '첫 결제 즉시, 실물 금이 적립됩니다. 월 적금액에 따라 5단계 등급으로 환영합니다.' : 'Physical gold is credited on your first payment. Welcome at one of 5 tiers based on your monthly savings.'}
          </p>
    ...
    <div style={{ background:'linear-gradient(150deg,#08070a,#0d0c12)', borderBottom:'1px solid rgba(197,165,114,0.12)', position:'relative', overflow:'hidden', minHeight:isMobile?'auto':480 }}>
      ...
      <div className="aurum-container" style={{ position:'relative', zIndex:1, paddingTop:isMobile?32:80, paddingBottom:isMobile?32:80, display:isMobile?'block':'flex', gap:56, alignItems:'center' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <p style={{ fontFamily:SANS, fontSize:15, color:'#a09080', lineHeight:1.9, marginBottom:28 }}>
            {ko ? '한번 얻은 할인은 절대 사라지지 않습니다. Founders Club의 다섯 문을 통과할 때마다, 할인율이 영구적으로 높아집니다. 귀하의 구매가 곧 귀하의 가격입니다.' : 'Each time your total purchases pass one of 5 thresholds, that discount stays with you for life.'}
          </p>
          <div style={{ marginBottom:32 }}>
            {gates.map((g, i) => (
...
// ═══════════════════════════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════════════════
// AURUM KOREA — NEW HOMEPAGE COMPONENTS
// Drop this entire block above `export default function HomePage` in
// src/pages/HomePage.jsx  (after the existing hook + constant definitions)
// ═══════════════════════════════════════════════════════════════════════

// ─── Historical data for chart components ────────────────────────────────
// Format: [year, month(1-12), xau_usd, krw_per_usd]
// IMPORTANT: Populate all 60 months Jan 2020 → Dec 2024 before deploying.
// Sample rows provided — extend with verified historical data.
const MONTHLY_DATA = [
...
// ─── Founders Club gate data (mirrors FoundersClubPage.jsx GATES) ────────
const FC_GATES = [
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// C·01  HeroSplitWidget — replaces HeroAnimatedVisual
// Props: prices, krwRate, lang, goldKR, goldAU
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  return (
    <div ref={ref} style={{ width:'100%', maxWidth:500, opacity:vis?1:0, transition:'opacity 0.7s ease' }}>
      ...
      {bars.map((bar, i) => (
        <div key={i} style={{ marginBottom:12 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:5 }}>
            ...
            <span style={{ fontFamily:MONO, fontSize:16, color:bar.color, fontWeight:700 }}>{fKRW(bar.val)}</span>
          </div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, background:'rgba(74,222,128,0.06)', border:'1px solid rgba(74,222,128,0.2)', padding:'10px 14px', marginTop:4 }}>
        ...
        <span style={{ fontFamily:MONO, fontSize:20, color:'#4ade80', fontWeight:700, marginLeft:'auto' }}>{fKRW(gap)}</span>
      </div>
      {prices?.silver && (
      ...
      <div style={{ fontFamily:MONO, fontSize:9, color:'#3a3028', marginTop:6, letterSpacing:'0.08em' }}>
        {ko ? '* 실시간 계산 · LBMA 현물가 기준' : '* Live calculation · LBMA spot basis'}
      </div>
    </div>
  );
}
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// C·02  KimchiPremiumMeter — SVG gauge, replaces stats bar
// Props: prices, krwRate, lang, goldKR, goldAU
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function KimchiPremiumMeter({ prices, krwRate, lang, goldKR, goldAU }) {
  ...
  // Gauge: -140° (left) to +140° (right), representing 0–25% premium range
  const norm = Math.min(Math.max(prem / 25, 0), 1);
  const deg = -140 + norm * 280;
  const rad = (deg - 90) * Math.PI / 180; // -90 offset: 0° points up
  ...
  const stats = [
    { l: ko ? '오늘' : 'Today',          v: prem.toFixed(1)+'%', c: zc },
  ...
  return (
    <div ref={ref} style={{ background:'#141414', borderBottom:'1px solid rgba(197,165,114,0.08)', padding:`${isMobile?20:24}px 0` }}>
      <div className="aurum-container" style={{ display:'flex', flexDirection:isMobile?'column':'row', alignItems:'center', justifyContent:'center', gap:40 }}>
        {/* SVG gauge */}
        <div style={{ position:'relative', width:200, height:130, flexShrink:0, minHeight:130 }}>
          <svg width="200" height="130" viewBox="0 0 200 130" style={{ overflow:'visible' }}>
            ...
            {vis && (
              <line x1="100" y1="105" x2={nx} y2={ny}
                stroke={zc} strokeWidth="2.5" strokeLinecap="round"
                style={{ transition:'x2 1s cubic-bezier(0.25,1,0.5,1), y2 1s cubic-bezier(0.25,1,0.5,1)' }}
              />
            )}
          ...
          <div style={{ position:'absolute', top:60, left:'50%', transform:'translateX(-50%)', textAlign:'center', minWidth:80 }}>
            <div style={{ fontFamily:MONO, fontSize:22, fontWeight:700, color:zc, lineHeight:1 }}>
              {vis ? prem.toFixed(1)+'%' : '—'}
            </div>
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// C·03  KumtongConverter — 금통장 gram comparison calculator
// Props: prices, krwRate, lang
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function KumtongConverter({ prices, krwRate, lang }) {
  ...
  // 시중 은행 금통장: 10% VAT on entry reduces effective gold ownership
  const kumtongGrams = amt / (pricePerGram * 1.10);
  // Aurum: transparent Aurum premium applied
    ...
    <div style={{ background:'rgba(197,165,114,0.04)', border:'1px solid rgba(197,165,114,0.18)', padding:isMobile?'20px 16px':'24px 28px', marginTop:20, position:'relative' }}>
      ...
      <div style={{ marginBottom:18 }}>
        ...
        <input type="range" min="1000000" max="100000000" step="1000000" value={amt}
          style={{ '--pct':`${((amt-1000000)/99000000*100).toFixed(1)}%`, width:'100%' }}
          onChange={e => setAmt(+e.target.value)} />
      </div>
      <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr', gap:10 }}>
        {[
          { label:ko?'시중 금통장 실질 보유량':'Bank gold account', grams:kumtongGrams, color:'#f87171', note:ko?'* VAT 10% 적용 후 실질 금량':'* After 10% VAT' },
          { label:ko?'Aurum 구매 시 보유량':'Aurum purchase', grams:aurumGrams, color:'#c5a572', note:ko?'* Aurum 수수료 적용 후':'* After Aurum fee' },
        ].map((item, i) => (
          <div key={i} style={{ background:'#141414', border:`1px solid ${item.color}44`, padding:'16px' }}>
            ...
            <div style={{ fontFamily:MONO, fontSize:28, color:item.color, fontWeight:700, lineHeight:1 }}>
              {fmt3(item.grams)}<span style={{ fontSize:14, marginLeft:4 }}>g</span>
            </div>
            <div style={{ fontFamily:MONO, fontSize:10, color:'#555', marginTop:4 }}>{(item.grams/OZ_IN_GRAMS).toFixed(4)} oz</div>
            <div style={{ fontFamily:SANS, fontSize:10, color:'#444', marginTop:6, fontStyle:'italic' }}>{item.note}</div>
          </div>
        ))}
      </div>
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// C·04  PersonalOverpayCalc — personal ₩ overpayment slider
// Props: goldKR, goldAU, lang, isMobile
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function PersonalOverpayCalc({ goldKR, goldAU, lang, isMobile }) {
  ...
  const fKRW = v => `₩${Math.round(v * mult).toLocaleString('ko-KR')}`;
  return (
    <div style={{ background:'#111', border:'1px solid rgba(197,165,114,0.2)', padding:isMobile?'20px 16px':'28px 32px', marginBottom:20, position:'relative' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:8 }}>
        ...
        <label style={{ fontFamily:SANS, fontSize:11, color:T.goldDim, display:'flex', alignItems:'center', gap:6, cursor:'pointer' }}>
          <input type="checkbox" checked={annual} onChange={e => setAnnual(e.target.checked)} />
          {ko ? '연간 환산 (×12)' : 'Annual (×12)'}
        </label>
      </div>
      <input type="range" min="1000000" max="100000000" step="1000000" value={amt}
        style={{ '--pct':`${((amt-1000000)/99000000*100).toFixed(1)}%`, width:'100%', marginBottom:18, minHeight:44 }}
        onChange={e => setAmt(+e.target.value)} />
      <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr 1fr':'1fr 1fr 1fr', gap:10 }}>
        <div style={{ background:'rgba(248,113,113,0.06)', border:'1px solid rgba(248,113,113,0.2)', padding:'14px 16px' }}>
          <div style={{ fontFamily:SANS, fontSize:11, color:'#f87171', marginBottom:4 }}>{ko ? '국내 소매 구매 시' : 'Korea retail cost'}</div>
          <div style={{ fontFamily:MONO, fontSize:16, color:'#f87171', fontWeight:700 }}>{fKRW(korCost)}</div>
        </div>
        <div style={{ background:'rgba(197,165,114,0.05)', border:'1px solid rgba(197,165,114,0.2)', padding:'14px 16px' }}>
          <div style={{ fontFamily:SANS, fontSize:11, color:'#c5a572', marginBottom:4 }}>Aurum</div>
          <div style={{ fontFamily:MONO, fontSize:16, color:'#c5a572', fontWeight:700 }}>{fKRW(amt)}</div>
        </div>
        <div style={{ background:'rgba(74,222,128,0.06)', border:'1px solid rgba(74,222,128,0.22)', padding:'14px 16px', gridColumn:isMobile?'1 / -1':'auto' }}>
          <div style={{ fontFamily:SANS, fontSize:11, color:'#4ade80', marginBottom:4 }}>{ko ? '절감액' : 'Savings'}</div>
          <div style={{ fontFamily:MONO, fontSize:isMobile?18:22, color:'#4ade80', fontWeight:700 }}>{fKRW(savings)}</div>
        </div>
      </div>
    </div>
  );
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// C·05  AGPAccumulatorCalc — 12-month bar chart inside AGPLaunchPane
// Props: prices, krwRate, lang, tiers
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function AGPAccumulatorCalc({ prices, krwRate, lang, tiers }) {
  ...
  const fKRW = v => `₩${Math.round(v).toLocaleString('ko-KR')}`;
  return (
    <div style={{ background:'rgba(14,12,8,0.8)', border:'1px solid rgba(197,165,114,0.15)', padding:isMobile?'18px 14px':'24px 24px', marginTop:20 }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:8 }}>
        <span style={{ fontFamily:MONO, fontSize:20, color:'#f5f0e8', fontWeight:700 }}>
        ...
        <span style={{ fontFamily:MONO, fontSize:11, color:'#c5a572', background:'rgba(197,165,114,0.1)', padding:'3px 10px' }}>
          {ko ? tier.name : tier.nameEn} {ko ? '티어' : 'tier'}
        </span>
      </div>
      <input type="range" min="200000" max="5000000" step="100000" value={monthly}
        style={{ '--pct':`${((monthly-200000)/4800000*100).toFixed(1)}%`, width:'100%', marginBottom:16, minHeight:44 }}
        onChange={e => setMonthly(+e.target.value)} />
      {/* 12-bar accumulation chart */}
      <div style={{ display:'flex', gap:3, alignItems:'flex-end', height:72, marginBottom:6 }}>
        {Array.from({ length:12 }, (_, i) => (
          <div key={i} style={{
            flex:1, height:`${Math.round(((i+1)/12) * 60)}px`,
            background: i===11 ? '#c5a572' : 'rgba(197,165,114,0.22)',
            transition:'height 0.4s ease', position:'relative'
          }}>
            {i===11 && (
              <div style={{ position:'absolute', bottom:'100%', left:'50%', transform:'translateX(-50%)', fontFamily:MONO, fontSize:9, color:'#c5a572', whiteSpace:'nowrap', marginBottom:2 }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', fontFamily:MONO, fontSize:9, color:'#444', marginBottom:14 }}>
        <span>{ko ? '1월' : 'M1'}</span><span>{ko ? '6월' : 'M6'}</span><span>{ko ? '12월' : 'M12'}</span>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:10 }}>
        {[
          { l:ko?'12개월 적립':'12-mo grams', v:totalGrams.toFixed(3)+'g', c:'#c5a572' },
          { l:ko?'국내 소매 환산':'Korea equiv.', v:fKRW(korEquiv), c:'#f87171' },
          { l:ko?'Aurum 총비용':'Aurum total', v:fKRW(monthly*12), c:T.goldDim },
          { l:ko?'절감액':'Savings', v:fKRW(savings), c:'#4ade80' },
        ].map((s,i) => (
          <div key={i} style={{ background:'#1a1a1a', padding:'10px 12px' }}>
      ...
      <div style={{ background:'rgba(74,222,128,0.05)', border:'1px solid rgba(74,222,128,0.2)', padding:'10px 14px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <span style={{ fontFamily:SERIF, fontStyle:'italic', fontSize:13, color:'#c5a572' }}>
          + {ko ? tier.name : tier.nameEn} {ko ? '첫달 즉시 적립' : 'Day-1 credit'}
        </span>
        <span style={{ fontFamily:MONO, fontSize:18, color:'#4ade80', fontWeight:700 }}>{tier.gift}</span>
      </div>
    </div>
  );
}
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// C·06  FoundersMiniCalc — gate estimator inside FoundersClubPane
// Props: krwRate, lang, navigate
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function FoundersMiniCalc({ krwRate, lang, navigate }) {
  ...
  const fKRW = v => `₩${Math.round(v).toLocaleString('ko-KR')}`;
  return (
    <div style={{ background:'rgba(197,165,114,0.04)', border:'1px solid rgba(197,165,114,0.15)', padding:'20px 20px', marginTop:20 }}>
      <div style={{ ...GOLD_LINE }} />
      <div style={{ fontFamily:MONO, fontSize:10, color:T.goldDim, letterSpacing:'0.18em', textTransform:'uppercase', marginBottom:14 }}>
        {ko ? '12개월 절감액 계산기' : 'Founders savings estimator'}
      </div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:8 }}>
        <span style={{ fontFamily:MONO, fontSize:18, color:'#f5f0e8' }}>
          ₩{Math.round(monthly/10000).toLocaleString()}{ko ? '만원/월' : '/mo'}
        </span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:12 }}>
        {[
          ...
          { l:ko?'연간 절감':'Ann. savings', v:gate?fKRW(savings):'−', c:'#4ade80' },
        ].map((s,i) => (
          <div key={i} style={{ background:'#1a1a1a', padding:'10px 12px', textAlign:'center' }}>
            <div style={{ fontFamily:SANS, fontSize:10, color:'#555', marginBottom:3 }}>{s.l}</div>
      ...
      <button onClick={() => navigate('founders')}
        style={{ background:'transparent', border:'1px solid rgba(197,165,114,0.4)', color:'#c5a572', padding:'11px 20px', fontFamily:SANS, fontSize:13, cursor:'pointer', width:'100%', minHeight:44 }}>
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// C·07  HowItWorks — 3-step process section
// Props: lang, isMobile
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function HowItWorks({ lang, isMobile }) {
  ...
  const steps = [
    { n:'STEP 01', t:ko?'신원 확인':'Verify Identity',
      d:ko?'NICE·KCB 본인인증 (1분). 국제 AML 기준 KYC. 한국 여권 또는 주민등록증.'
         :'NICE/KCB identity check (1 min). International AML KYC. Korean passport or national ID.' },
    { n:'STEP 02', t:ko?'구매 또는 적립':'Purchase or Accumulate',
      d:ko?'토스뱅크 자동이체, 카드, 국제 송금. 원화로 결제, LBMA 국제 현물가로 실시간 환산.'
         :'Toss auto-debit, card, or wire. Pay in KRW, converted at live LBMA spot.' },
    { n:'STEP 03', t:ko?'싱가포르 금보관 배분':'Singapore Vault Allocation',
      d:ko?'결제 즉시 싱가포르 Malca-Amit FTZ 해외 금보관 금고에 귀하의 명의로 배분. 고유 일련번호 발급.'
         :'Immediately allocated to your name at Malca-Amit Singapore FTZ vault. Serial number issued.' },
  ];
  return (
    <div ref={ref} style={{ borderBottom:'1px solid rgba(197,165,114,0.08)', opacity:vis?1:0, transform:vis?'translateY(0)':'translateY(20px)', transition:'opacity 0.7s ease, transform 0.7s ease' }}>
      <div className="aurum-container" style={{ paddingTop:isMobile?32:64, paddingBottom:isMobile?32:64 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr 1fr', gap:12 }}>
          {steps.map((s,i) => (
            <div key={i} style={{
              background:'rgba(197,165,114,0.03)', border:'1px solid rgba(197,165,114,0.12)',
              borderTop:'2px solid #c5a572', padding:'18px 18px',
        ...
        <div style={{ marginTop:14, padding:'10px 14px', background:'rgba(74,222,128,0.07)', border:'1px solid rgba(74,222,128,0.2)', fontFamily:MONO, fontSize:11, color:'#4ade80', letterSpacing:'0.06em', textAlign:'center' }}>
          ✓ {ko?'100g 도달 시 실물 바 무료 전환':'Free bar at 100g'} &nbsp;·&nbsp; ✓ {ko?'원화 즉시 정산 가능':'Instant KRW settlement'} &nbsp;·&nbsp; ✓ {ko?'언제든 중단 가능':'Cancel anytime'}
        </div>
      </div>
    </div>
  );
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// C·08  LegalSafetyFAQ  ⚠️ GC_REVIEW_REQUIRED before publishing
// Props: lang, navigate
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function LegalSafetyFAQ({ lang, navigate }) {
  ...
  const faqs = [
    { v: ko?'✓ 합법 확인':'✓ Confirmed Legal',
      ...
      a: ko?'네. 외국환거래법상 연간 $50,000 이하의 해외 투자는 사전 신고 없이 가능합니다. Aurum은 싱가포르 귀금속서비스법(PSPM Act) 2019에 등록된 공식 딜러입니다. 해외 금융 계좌 잔고가 $10,000 이상이면 매년 6월 국세청에 신고하셔야 합니다. Aurum은 이 신고에 필요한 계좌 정보를 자동으로 제공합니다.'
         :'Yes. Under Korean foreign exchange law, overseas investments up to $50,000/year require no prior notification. Aurum is a registered dealer under Singapore PSPM Act 2019. Annual reporting to Korean tax authorities applies if your overseas balance exceeds $10,000 — Aurum provides the required information automatically.' },
    { v: ko?'✓ 물리적으로 존재':'✓ Physically Exists',
      q: ko?'금이 실제로 금고에 있나요?':'Is the gold physically in a vault?',
      a: ko?'귀하의 금은 싱가포르 자유무역지대(FTZ) Malca-Amit 해외 금보관 금고에 물리적으로 보관됩니다. 완전 배분(fully allocated) 방식으로 다른 고객 자산과 절대 혼합되지 않습니다. 구매 즉시 고유 일련번호가 발급되며, 독립 감사 기관이 매일 보유 현황을 검증합니다.'
         :'Your metal is physically stored at the Malca-Amit Singapore FTZ vault under fully allocated custody — never commingled with other clients. A unique serial number is issued immediately, with daily independent verification.' },
    { v: ko?'✓ 법적 분리 보장':'✓ Legally Separated',
      q: ko?'Aurum이 폐업하면 내 금은 어떻게 되나요?':'What happens if Aurum closes?',
      a: ko?'배분 보관 구조상, 귀하의 금은 법적으로 Aurum의 자산이 아닙니다. Aurum이 폐업하더라도 귀하의 금속은 귀하 명의로 Malca-Amit 금고에 존재하며, 귀하는 해당 금속에 대한 소유권을 보유합니다. 이것이 시중 은행 금통장과의 근본적 차이입니다 — 금통장은 은행의 부채이지만, Aurum 계좌의 금속은 귀하의 재산입니다.'
         :'Under allocated storage, your gold is legally not an asset of Aurum. Even if Aurum closes, your metal remains in your name at the Malca-Amit vault. This is the fundamental difference from a bank gold account — bank accounts are bank liabilities; Aurum allocated metal is your property.' },
  ];
  return (
    <div ref={ref} style={{ borderBottom:'1px solid rgba(197,165,114,0.08)', opacity:vis?1:0, transition:'opacity 0.8s ease' }}>
      <div className="aurum-container" style={{ paddingTop:isMobile?32:64, paddingBottom:isMobile?32:64 }}>
        ...
        {faqs.map((faq, i) => (
          <div key={i} style={{ borderBottom:i<2?'1px solid rgba(197,165,114,0.08)':'none', paddingBottom:20, marginBottom:20 }}>
            ...
            <button onClick={() => setOpen(open===i ? null : i)}
              style={{ background:'none', border:'none', textAlign:'left', cursor:'pointer', padding:0, width:'100%', display:'flex', justifyContent:'space-between', alignItems:'center', gap:12, minHeight:44 }}>
        ...
        <button onClick={() => navigate('storage')}
          style={{ background:'transparent', border:'1px solid rgba(197,165,114,0.3)', color:'#c5a572', padding:'11px 20px', fontFamily:SANS, fontSize:13, cursor:'pointer', minHeight:44 }}>
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// C·09  GoldKRWIndexedChart — dual-line indexed performance
// Props: lang, isMobile
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function GoldKRWIndexedChart({ lang, isMobile }) {
  ...
  useEffect(() => {
    ...
    const ctx = canvas.getContext('2d');
    const W = canvas.width, H = canvas.height;
    const base = MONTHLY_DATA[0];
    const pts = MONTHLY_DATA.map((d, i) => ({
      gold: (d[2] * d[3]) / (base[2] * base[3]) * 100,
      krw:  (base[3] / d[3]) * 100,
      bank: Math.pow(1 + 0.035/12, i) * 100,
    }));
    const allVals = pts.flatMap(p => [p.gold, p.krw, p.bank]);
    const minV = Math.min(...allVals) * 0.97, maxV = Math.max(...allVals) * 1.02;
    const toX = i => 44 + (i / (pts.length-1)) * (W - 56);
    const toY = v => H*0.88 - ((v - minV) / (maxV - minV)) * (H * 0.76);
    ctx.clearRect(0, 0, W, H);
    // Grid
    [100, 125, 150, 175, 200, 250].forEach(v => {
      if (v < minV || v > maxV) return;
      ctx.strokeStyle = 'rgba(197,165,114,0.05)'; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.moveTo(44, toY(v)); ctx.lineTo(W-12, toY(v)); ctx.stroke();
      ctx.fillStyle = 'rgba(138,125,107,0.4)'; ctx.font = `9px 'JetBrains Mono'`;
      ctx.fillText(v, 4, toY(v)+3);
    });
    // Lines
    [['gold','#C5A572',false],['krw','#f87171',false],['bank','rgba(138,125,107,0.5)',true]].forEach(([key,col,dashed]) => {
      ctx.beginPath(); ctx.strokeStyle = col; ctx.lineWidth = key==='bank'?1:2; ctx.lineJoin = 'round';
      if (dashed) ctx.setLineDash([4,3]); else ctx.setLineDash([]);
      pts.forEach((p,i) => i===0 ? ctx.moveTo(toX(i),toY(p[key])) : ctx.lineTo(toX(i),toY(p[key])));
      ctx.stroke();
    });
    ctx.setLineDash([]);
    // Legend
    [[ko?'금 (KRW 환산)':'Gold (KRW)','#C5A572'],[ko?'KRW 구매력':'KRW power','#f87171'],[ko?'은행 예금 3.5%':'Bank 3.5%','rgba(138,125,107,0.7)']].forEach(([label,col],i) => {
      ctx.strokeStyle = col; ctx.lineWidth = i===2?1:2; if(i===2) ctx.setLineDash([4,3]); else ctx.setLineDash([]);
      ctx.beginPath(); ctx.moveTo(W-116, 14+i*16); ctx.lineTo(W-96, 14+i*16); ctx.stroke();
      ctx.setLineDash([]); ctx.fillStyle = 'rgba(138,125,107,0.65)'; ctx.font = `9px 'JetBrains Mono'`;
      ctx.fillText(label, W-90, 17+i*16);
    });
    ctx.fillStyle = 'rgba(197,165,114,0.3)'; ctx.font = `9px 'JetBrains Mono'`;
    ctx.fillText('Jan 2020 = 100', 44, H-3);
  }, [vis, lang]);
  return (
    <div ref={ref} style={{ opacity:vis?1:0, transition:'opacity 0.8s ease' }}>
      <div style={{ ...eyebrowStyle, marginBottom:8 }}>
        {ko ? '2020년 1월 = 100 인덱스' : 'Indexed: Jan 2020 = 100'}
      </div>
      <canvas ref={canvasRef} width={isMobile?320:480} height={240}
        style={{ display:'block', width:'100%', maxWidth:isMobile?320:480 }} />
    </div>
  );
}

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// C·10  SavingsComparisonChart — bank vs AGP gold accumulation
// Props: prices, krwRate, lang, isMobile
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function SavingsComparisonChart({ prices, krwRate, lang, isMobile }) {
  ...
  const [ref, vis] = useScrollReveal(0.05);
  const ko = lang === 'ko';
  const curPricePerG = prices.gold * krwRate / OZ_IN_GRAMS;
  useEffect(() => {
    if (!vis || !canvasRef.current) return;
    const canvas = canvasRef.current;
    const ctx = canvas.getContext('2d');
    ...
    MONTHLY_DATA.forEach((d, i) => {
      bankTotal += monthly * Math.pow(1 + 0.035/12, MONTHLY_DATA.length - 1 - i);
      grams += monthly / (d[2] * d[3] / OZ_IN_GRAMS);
      bankPts.push(bankTotal);
      goldPts.push(grams * curPricePerG);
    });
    const allV = [...bankPts, ...goldPts];
    const minV = Math.min(...allV)*0.96, maxV = Math.max(...allV)*1.02;
    const toX = i => 8 + (i/(MONTHLY_DATA.length-1)) * (W-16);
    const toY = v => H*0.9 - ((v-minV)/(maxV-minV)) * (H*0.78);
    ctx.clearRect(0, 0, W, H);
    [[bankPts,'rgba(138,125,107,0.5)',false],[goldPts,'#C5A572',true]].forEach(([pts,col,fill]) => {
      if (fill) {
        const grad = ctx.createLinearGradient(0, toY(Math.max(...pts)), 0, H*0.9);
        grad.addColorStop(0, 'rgba(197,165,114,0.15)'); grad.addColorStop(1, 'rgba(197,165,114,0)');
        ctx.beginPath(); ctx.moveTo(toX(0), H*0.9);
        pts.forEach((v,i) => ctx.lineTo(toX(i), toY(v)));
        ctx.lineTo(toX(pts.length-1), H*0.9); ctx.closePath(); ctx.fillStyle = grad; ctx.fill();
      }
    ...
    ctx.fillStyle = 'rgba(74,222,128,0.12)'; ctx.fillRect(W-88, 8, 80, 32);
    ctx.fillStyle = '#4ade80'; ctx.font = `bold 10px 'JetBrains Mono'`;
    ctx.fillText(ko?'금 수익':'Gold gain', W-82, 21);
    ctx.font = `9px 'JetBrains Mono'`;
    ctx.fillText(`+₩${Math.round(diff/10000).toLocaleString()}만`, W-82, 34);
  }, [vis, monthly, prices.gold, krwRate, lang]);
  return (
    <div ref={ref} style={{ opacity:vis?1:0, transition:'opacity 0.8s ease' }}>
      <div style={{ fontFamily:MONO, fontSize:10, color:T.goldDim, letterSpacing:'0.16em', textTransform:'uppercase', marginBottom:8, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <span>{ko ? '은행 예금 vs 금 적립' : 'Bank savings vs Gold AGP'}</span>
      ...
      <input type="range" min="50000" max="1000000" step="50000" value={monthly}
        style={{ '--pct':`${((monthly-50000)/950000*100).toFixed(1)}%`, width:'100%', marginBottom:8, minHeight:44 }}
        onChange={e => setMonthly(+e.target.value)} />
      <canvas ref={canvasRef} width={isMobile?300:420} height={200}
        style={{ display:'block', width:'100%', maxWidth:isMobile?300:420 }} />
      <div style={{ display:'flex', gap:14, marginTop:6, fontFamily:MONO, fontSize:9, color:'#555' }}>
        <span style={{ color:'#c5a572' }}>━ {ko?'금 (현재가 기준)':'Gold (current price)'}</span>
        <span>━ {ko?'은행 예금 3.5%':'Bank 3.5%/yr'}</span>
      </div>
      <div style={{ fontFamily:SANS, fontSize:10, color:'#3a3028', marginTop:4, fontStyle:'italic' }}>
        * {ko?'과거 성과가 미래 수익을 보장하지 않습니다':'Past performance is not indicative of future results'}
      </div>
    </div>
  );
}
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// C·11  VaultLocationDot — static SVG above trust partner grid
// Props: lang
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:20, marginBottom:24, padding:'14px 0', borderBottom:'1px solid rgba(197,165,114,0.06)', flexWrap:'wrap' }}>
      <svg width="180" height="80" viewBox="0 0 180 80" style={{ flexShrink:0 }}>
        <style>{`
          @keyframes sgpulse { 0%,100%{r:8;opacity:0.15} 50%{r:14;opacity:0.05} }
          .sg-ring { animation: sgpulse 2.5s ease-in-out infinite; }
        `}</style>
        ...
        <circle cx="116" cy="54" className="sg-ring" fill="none" stroke="rgba(197,165,114,0.28)" strokeWidth="1" r="8"/>
      ...
      <div>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          {[ko?'40년+ 운영':'40+ years', ko?"Lloyd's 보험":"Lloyd's insured", ko?'자유무역지대':'Free Trade Zone', ko?'금보관 전문':'Precious metals specialist'].map((b,i) => (
            <span key={i} style={{ fontFamily:MONO, fontSize:10, color:T.goldDim, border:'1px solid rgba(197,165,114,0.15)', padding:'2px 8px', letterSpacing:'0.08em' }}>{b}</span>
          ))}
        </div>
      </div>
    </div>
...
export default function HomePage({ navigate, prices, krwRate, currency, setCurrency, lang }) {
  ...
  const whyItems = [
    { icon:<IconBox><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/><circle cx="12" cy="16" r="1.5" fill="currentColor" stroke="none"/><line x1="12" y1="17.5" x2="12" y2="19"/></svg></IconBox>, title: lang==='ko'?'완전 배분 보관 — 귀하의 금속, 귀하의 이름':'Fully Allocated Storage — Your Metal, Your Name', content: lang==='ko'?'귀하의 금속은 다른 고객 자산과 절대 혼합되지 않습니다. 싱가포르 Malca-Amit FTZ 해외 금보관 금고에 고유 일련번호와 함께 귀하의 명의로 영구 등록됩니다. 시중 금통장과 달리, 귀하의 금은 어떤 기관의 자산도 아닙니다 — 법적으로 귀하의 소유입니다.':'Your metal is never commingled with other clients. Registered in your name with a unique serial number at the Malca-Amit Singapore FTZ vault.' },
    { icon:<IconBox><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/><line x1="2" y1="20" x2="22" y2="20" strokeDasharray="2 2"/></svg></IconBox>, title: lang==='ko'?'국제 현물가 직거래 — 김치 프리미엄 없음':'International Spot Price — No Kimchi Premium', content: lang==='ko'?'시중 금통장, 국내 귀금속 딜러 — 모두 국제 현물가 대비 유의미한 프리미엄이 구조적으로 포함됩니다. Aurum은 LBMA 국제 현물가 기준의 투명한 수수료만 적용합니다. 숨겨진 비용 없음.':'Korean retail (KRX, bank gold accounts) carries ~20% above international spot. Aurum: LBMA spot + 8% (gold) / 15% (silver) transparent premium.' },
    ...
    <div>
          ...
          <div style={{ flex:1, maxWidth:isMobile?'100%':520 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:20, flexWrap:'nowrap', overflow:'hidden' }}>
              ...
              <span style={{ fontFamily:SERIF, fontStyle:'italic', fontSize:isMobile?12:14, color:'#c5a572', letterSpacing:'0.04em', whiteSpace:'nowrap' }}>
                {lang==='ko'?'싱가포르 PSPM법 2019 등록 딜러':'Registered PSPM Act 2019 · Singapore'}
              </span>
            ...
            <h1 style={{ fontFamily:lang==='ko'?T.serifKrDisplay:SERIF, fontSize:isMobile?32:54, fontWeight:300, color:'#f5f0e8', lineHeight:1.1, margin:'0 0 20px' }}>
              {lang==='ko'
                ?(<><span style={{ color:'#c5a572', fontWeight:400, fontStyle:'italic', fontFamily:T.serif }}>오늘도 내고 계십니까?</span><br />오늘부터 바꾸십시오.</>)
            ...
            <p style={{ fontFamily:SANS, fontSize:isMobile?14:16, color:'#a09080', lineHeight:1.95, margin:'0 0 10px' }}>
              {lang==='ko'
                ...
                :'Not an ETF. Not a bank account. Physical gold in a Singapore vault, in your name — without Korea\'s 20% price markup.'}
            </p>
            ...
            <div style={{ display:'flex', gap:12, flexDirection:isMobile?'column':'row' }}>
              <button onClick={() => navigate('shop-physical')} style={{ flex:1, background:'linear-gradient(135deg,#c5a572,#8a6914)', color:'#fff', border:'none', padding:'14px 20px', fontSize:15, fontFamily:SANS, fontWeight:700, borderRadius:0, cursor:'pointer' }}>
                {lang==='ko'?'국제 현물가로 시작하기 →':'Buy at LBMA Spot →'}
              </button>
              <button onClick={() => navigate('agp')} style={{ flex:1, background:'transparent', color:'#a09080', border:'1px solid #282828', padding:'14px 20px', fontSize:15, fontFamily:SANS, fontWeight:600, borderRadius:0, cursor:'pointer' }}>
                {lang==='ko'?'다음 달부터 자동으로 · ₩200K/월':'Auto-accumulate · ₩200K/mo'}
              </button>
            </div>
            <div style={{ marginTop:16, display:'grid', gridTemplateColumns:isMobile?'1fr 1fr':'repeat(4,1fr)', gap:8 }}>
              {[
                { top:'🇸🇬 PSPM 2019', bot:lang==='ko'?'싱가포르 금융청 등록':'MAS Registered' },
                { top:'Malca-Amit FTZ', bot:lang==='ko'?'해외 금보관 전문':'Singapore Vault' },
                { top:"Lloyd's of London", bot:lang==='ko'?'기관급 전액 보험':'Full Insurance' },
                { top:'LBMA 승인', bot:lang==='ko'?'PAMP · Argor-Heraeus':'PAMP · Argor' },
              ].map((b,i)=>(
                <div key={i} style={{ border:'1px solid rgba(197,165,114,0.15)', padding:'6px 10px', background:'rgba(197,165,114,0.03)' }}>
                  <div style={{ fontFamily:MONO, fontSize:isMobile?9:10, color:'#c5a572', fontWeight:700, letterSpacing:'0.06em', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{b.top}</div>
                  <div style={{ fontFamily:SANS, fontSize:isMobile?9:10, color:'#555', marginTop:2 }}>{b.bot}</div>
                </div>
              ))}
            </div>
            <div style={{ fontFamily:MONO, fontSize:10, color:'#3a3028', marginTop:8, letterSpacing:'0.08em' }}>
              {lang==='ko'?'귀하의 금은 귀하의 금입니다.':'귀하의 금은 귀하의 금입니다.'}
            </div>
          </div>
          {/* Right: HeroSplitWidget — live KRW price bars */}
          {!isMobile && (
            <div style={{ flex:1, display:'flex', justifyContent:'center', alignItems:'center' }}>
      ...
      {(() => {
        ...
        const pvpRows = [
          ...
          { label: lang==='ko'?'배분 보관':'Allocated Storage',         bad: lang==='ko'?'✗ 혼합 보관 · 재담보 위험':'✗ Pooled / rehypothecated', good: lang==='ko'?'✓ 완전 배분':'✓ Fully Segregated' },
          { label: lang==='ko'?'파산 보호':'Insolvency Protection',     bad: lang==='ko'?'✗ 예금자보호 적용 불가':'✗ Not deposit-protected',                   good: lang==='ko'?'✓ 법적 분리':'✓ Legally Separated' },
          { label: lang==='ko'?'일련번호':'Serial Number',              bad: lang==='ko'?'✗ 없음':'✗ None',                           good: lang==='ko'?'✓ 고유 식별':'✓ Unique ID' },
          { label: lang==='ko'?'실물 인출':'Physical Withdrawal',       bad: lang==='ko'?'제한적 · 추가 부가세 부과':'Restricted + VAT on exit',           good: lang==='ko'?'✓ 언제든 가능':'✓ Anytime' },
          { label: lang==='ko'?'관할권 분산':'Jurisdiction',            bad: lang==='ko'?'국내 규제만 · FX 노출':'Korea only · FX exposed',                    good: lang==='ko'?'싱가포르 + 한국 이중':'Singapore + Korea Dual' },
        ];
        return (
        ...
        <div className="aurum-container" style={{ paddingTop: isMobile ? 32 : 72, paddingBottom: isMobile ? 32 : 64 }}>
          <div style={{ textAlign: 'center', marginBottom: isMobile ? 24 : 40 }}>
            <span style={{ ...eyebrowStyle, display: 'block', textAlign: 'center' }}>{lang === 'ko' ? '금통장을 가지고 계십니까?' : 'The Fundamental Difference'}</span>
            ...
            <div style={{ background: 'rgba(248,113,113,0.03)', border: '1px solid rgba(248,113,113,0.2)', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ padding: isMobile ? '20px 18px' : '28px 28px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  ...
                  <div>
                    <div style={{ fontFamily: MONO, fontSize: 10, color: '#f87171', letterSpacing: '0.16em', textTransform: 'uppercase' }}>{lang === 'ko' ? '시중 금통장 · ETF · 선물' : 'Paper Gold / Silver'}</div>
                    <div style={{ fontFamily: SERIF, fontStyle: 'italic', fontSize: 13, color: 'rgba(248,113,113,0.6)' }}>{lang === 'ko' ? '"청구권을 보유합니다 — 실물이 아닙니다"' : '"You hold a claim"'}</div>
                  </div>
                </div>
      ...
      {/* §02b KumtongConverter — 금통장 vs Aurum gram comparison */}
      <div style={{ background:'#0d0b08', borderBottom:'1px solid rgba(197,165,114,0.08)' }}>
        <div className="aurum-container" style={{ paddingTop:isMobile?24:48, paddingBottom:isMobile?24:48 }}>
          <KumtongConverter prices={prices} krwRate={krwRate} lang={lang} />
          <div style={{ marginTop:16, textAlign:'right' }}>
            <button onClick={() => navigate('storage')} style={{ background:'transparent', border:'1px solid rgba(197,165,114,0.3)', color:'#c5a572', padding:'10px 22px', cursor:'pointer', fontFamily:SANS, fontSize:13 }}>
              {lang==='ko'?'전체 비교 분석 보기 →':'Full custody comparison →'}
            </button>
          </div>
        </div>
      </div>

      {/* §03 TRUST WALL — why you can trust Aurum */}
      {/* ⑧ Trust section — expanded premium partner section */}
      {(() => {
        const [ref, vis] = useScrollReveal(0.1);
        const partners = [
          {
            abbr: 'SG', name: 'Malca-Amit FTZ', sub: lang==='ko'?'40년 귀금속 물류 선두. 싱가포르 FTZ 해외 금보관 전문.':'40yr precious metals leader. Singapore FTZ offshore vault specialist.',
          ...
          {
            abbr: "LL", name: "Lloyd's of London", sub: lang==='ko'?'전 세계 해외 보관 금의 보험 기준. Aurum 보유 금속 전액 커버.':'Global standard for offshore stored gold. Full coverage on all Aurum metal.',
          ...
          {
            abbr: 'LB', name: 'LBMA', sub: lang==='ko'?'국제 귀금속 거래의 글로벌 기준. Aurum은 LBMA 승인 바만 취급합니다.':'Global standard for precious metals. Aurum handles LBMA-approved bars only.',
          ...
          {
            abbr: 'SG', name: 'PSPM 2019 · MAS', sub: lang==='ko'?'싱가포르 PSPM법 2019 등록 딜러. MAS 규정 AML/KYC 완전 준수.':'PSPM Act 2019 registered dealer. Full MAS AML/KYC compliance.',
          ...
          {
            abbr: 'AU', name: lang==='ko'?'매일 감사':'Daily Audit', sub: lang==='ko'?'독립 감사 기관 일일 검증. 레버리지·재담보 없음.':'Daily independent verification. Zero leverage or rehypothecation.',
      ...
      {/* §04 KIMCHI PREMIUM METER — replaces stats bar */}
      ...
      {(() => {
        ...
        <div className="aurum-container" style={{ paddingTop: isMobile ? 40 : 72, paddingBottom: isMobile ? 32 : 60 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
            <div>
              <span style={eyebrowStyle}>{lang === 'ko' ? '국내 금 구매의 실제 비용' : 'The Real Cost of Buying Gold in Korea'}</span>
              <h2 style={{ fontFamily: SERIF, fontSize: isMobile ? 28 : 38, color: '#f5f0e8', fontWeight: isMobile ? 300 : 400, margin: 0 }}>{lang === 'ko' ? `₩1,000만원 투자 시, 약 ₩${Math.round((goldKR-goldAU)/goldAU*10000000/10000).toLocaleString('ko-KR')}만원을 더 내게 됩니다.` : `On ₩10M invested, that's roughly ₩${Math.round((goldKR-goldAU)/goldAU*10000000/10000).toLocaleString('ko-KR')} more than necessary.`}</h2>
              <p style={{ fontFamily: SANS, fontSize: 13, color: '#8a7d6b', margin: '6px 0 0' }}>{lang === 'ko' ? 'Aurum 매입가 vs 국내 소매가 기준 (부가세 포함)' : 'Aurum Price vs Korean Retail (domestic premium + VAT)'}</p>
            </div>
      ...
      {/* §06 AGP LAUNCH — moved after savings per architecture */}
      ...
      {/* §09 LEGAL FAQ — GC_REVIEW_REQUIRED before publish */}
      <LegalSafetyFAQ lang={lang} navigate={navigate} />

      <SealDivider />


      {/* §10 MARKET ANALYTICS + 2 charts — between Legal FAQ and Why Aurum */}
      {(() => {
        return (
        ...
        <div className="aurum-container" style={{ paddingTop: isMobile ? 32 : 64, paddingBottom: isMobile ? 32 : 64 }}>
          <div style={{ textAlign:'center', marginBottom: isMobile?20:36 }}>
            <span style={{ fontFamily:MONO, fontSize:10, color:'rgba(197,165,114,0.5)', letterSpacing:'0.2em', textTransform:'uppercase' }}>
              {lang==='ko'?'시장이 보여주는 것':'What the Market Shows'}
            </span>
            <h2 style={{ fontFamily:SERIF, fontStyle:'italic', fontSize: isMobile?22:32, color:'#f5f0e8', fontWeight:300, margin:'8px 0 0' }}>
              {lang==='ko'
                ?<>시장이 보여주는 것 — <span style={{ color:'#c5a572' }}>직접 확인하십시오</span></>
                :<>The data says — <span style={{ color:'#c5a572' }}>see for yourself</span></>}
            </h2>
          </div>
          ...
          <div style={{ marginTop:20, textAlign:'center' }}>
            <button onClick={() => navigate('register')} style={{ background:'#c5a572', border:'none', color:'#0a0a0a', padding:'14px 36px', fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:SANS }}>
              {lang==='ko'?'가입하고 직접 확인하기 →':'Join and see for yourself →'}
            </button>
          </div>
        </div>
        </div>
          ...
          <div style={{ display: isMobile ? 'block' : 'flex', gap: 64, alignItems: 'center' }}>
            ...
            <div style={{ flex: '1 1 0', minWidth: 0 }}>
              <div style={{ marginBottom: 28 }}>
                <span style={eyebrowStyle}>{lang === 'ko' ? 'Aurum이 다른 이유' : 'Why Choose Aurum'}</span>
                <h2 style={{ fontFamily: SERIF, fontSize: isMobile ? 28 : 38, color: '#f5f0e8', fontWeight: isMobile ? 300 : 400, margin: 0 }}>{lang === 'ko' ? '5가지 확실한 근거' : 'The Aurum Difference'}</h2>
              </div>
      ...
      {(() => {
        ...
        <div className="aurum-container">
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            ...
            <span style={{ ...eyebrowStyle, display: 'block', textAlign: 'center' }}>{lang === 'ko' ? '세 가지 방법' : 'How to Start'}</span>
            <h2 style={{ fontFamily: SERIF, fontSize: isMobile ? 28 : 38, color: '#f5f0e8', fontWeight: isMobile ? 300 : 400, margin: 0 }}>{lang === 'ko' ? '지금, 세 가지 방법으로.' : 'How Would You Like to Begin?'}</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr', gap: 14 }}>
            {[
              ...
              { iconLines: ['FC'], badge: lang === 'ko' ? '누적 할인 멤버십' : 'Cumulative Discount', title: 'Founders Club', desc: lang === 'ko' ? '구매 누적에 따라 할인율이 영구적으로 높아집니다. 한번 얻은 할인은 사라지지 않습니다.' : 'Your discount grows with cumulative purchases. Earned discounts never expire.', bullets: lang === 'ko' ? ['5개 게이트 · 누적 GMV 기준', '한번 얻은 할인 영구 유지', 'HNW 고액 투자자'] : ['5 gates · cumulative GMV', 'Earned discounts never expire', 'HNW premium investors'], cta: lang === 'ko' ? 'Founders Club 알아보기' : 'Explore Founders', route: 'founders', featured: false },
            ].map((c, i) => (
