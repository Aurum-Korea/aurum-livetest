  ...
  return (
    <section style={{ padding: '80px 24px 48px', maxWidth: 1280, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        {[
          { lbl: '친구당 · Per invite', v: '₩50,000', sub: '실물 금 · 두 사람 모두', accent: P.hot },
...
function EarningsCalc() {
  ...
  const calc = useMemo(() => {
    // Bounty per friend: ₩50K each side × friends = ₩50K × friends for me
    const signupBounty = 50_000 * friends;
    // 5% of each friend's monthly → my GMV per month. Over 12 months = annual.
    const gmvFromReferrals = avgMonthly * friends * 12;
    const gmvCredit = gmvFromReferrals * 0.05;   // 5% counts toward MY GMV
    // Find tier I'd unlock at that GMV
    let unlockedTier = null;
    for (let i = GATES.length - 1; i >= 0; i--) {
      if (gmvFromReferrals >= GATES[i].gmv) { unlockedTier = GATES[i]; break; }
    }
    const total = signupBounty + (unlockedTier ? 400_000 : 0);
    return { signupBounty, gmvFromReferrals, gmvCredit, unlockedTier, total };
  }, [friends, avgMonthly]);
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 22, marginBottom: 24 }}>
          <div>
            ...
            <input type="range" min="1" max="50" step="1" value={friends}
              ...
              style={{ width: '100%', accentColor: P.hot }} />
        ...
        <div style={{
          ...
          {[
            ...
            { lbl: '친구 연간 GMV', v: fmtMan(calc.gmvFromReferrals), sub: '12개월 기준', color: P.chrome },
            { lbl: '내 GMV 크레딧 · 5%', v: fmtMan(calc.gmvCredit), sub: '내 게이트로 반영', color: P.blueB },
