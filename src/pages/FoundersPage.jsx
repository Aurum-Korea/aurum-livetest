            ...
            {GATES.map((g, i) => {
              ...
              return (
                <button key={i} onClick={() => setActiveIdx(i)} style={{
                  ...
                  <div style={{
                    width: active ? 56 : 44, height: active ? 56 : 44, borderRadius: '50%',
                    margin: '0 auto 14px',
                    background: g.apex ? T.gold : (active ? T.goldGlow : T.bg),
                    border: `1.5px solid ${g.apex ? T.goldB : (active ? T.goldBorderS : T.goldBorder)}`,
                    boxShadow: active ? `0 0 24px rgba(197,165,114,0.4)` : 'none',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontFamily: T.serif, fontStyle: 'italic',
                    fontSize: active ? 22 : 18,
                    color: g.apex ? T.bg : (active ? T.goldB : T.gold),
                    fontWeight: 400, transition: 'all 0.35s cubic-bezier(0.2,0.8,0.2,1)',
                    position: 'relative', zIndex: 1,
                  }}>{g.n}</div>
                  <div style={{ fontFamily: T.serifKr, fontSize: 13, color: active ? T.text : T.sub, fontWeight: 500, lineHeight: 1.3 }}>{g.ko}</div>
                  <div style={{ fontFamily: T.serif, fontStyle: 'italic', fontSize: 10, color: T.goldD, marginTop: 2 }}>{g.en}</div>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 14, paddingTop: 18, borderTop: `1px dashed ${T.border}` }}>
                <div>
                  ...
                  <div style={{ fontFamily: T.mono, fontSize: 15, color: T.text, fontWeight: 500 }}>₩{(GATES[activeIdx].gmv / 10000).toLocaleString()}만</div>
                </div>
                <div>
            ...
            <div style={{ padding: 30, borderBottom: `1px solid ${T.border}` }}>
              ...
              <div style={{ background: T.deepBlack, border: `1px dashed ${T.border}`, padding: 16, fontFamily: T.mono, fontSize: 11 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <span style={{ color: T.muted, letterSpacing: '0.2em', fontSize: 9 }}>CUMULATIVE GMV</span>
                  <span style={{ color: T.text, fontWeight: 600 }}>₩{(sim.totalGMV / 10000).toLocaleString()}만</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <span style={{ color: T.muted, letterSpacing: '0.2em', fontSize: 9 }}>PROJECTED @ {(cagr * 100).toFixed(0)}%</span>
                  <span style={{ color: T.goldB, fontWeight: 600 }}>₩{(sim.projectedValue / 10000).toLocaleString('en-US', { maximumFractionDigits: 0 })}만</span>
                </div>
...
function TransitionStripe() {
  return (
    <section style={{
      ...
      <div style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: 1, background: `linear-gradient(90deg, transparent 10%, ${T.gold} 50%, transparent 90%)`, transform: 'translateY(-20px)', opacity: 0.5 }} />
      <div style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: 1, background: `linear-gradient(90deg, transparent 10%, ${T.gold} 50%, transparent 90%)`, transform: 'translateY(20px)', opacity: 0.5 }} />
      {/* Seal ornament */}
      <div style={{
        ...
        <div style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontFamily: T.serif, fontStyle: 'italic', color: T.bg, fontSize: 14, fontWeight: 600,
        }}>A</div>
      </div>
  ...
  const marks = [
    {
      ...
      spec: '10K · 12g · 이니셜 각인',
      desc: '10K 솔리드 골드. 손바닥 크기. 각 마크는 고유 시리얼. 도장 인식 번호는 Aurum의 영구 기록부에 남습니다.',
    },
    {
      gate: 'GATE V',
      name: '평생 각인 마크',
      nameEn: 'Lifetime Engraved Mark',
      spec: '24K · 50g · 각인 이니셜',
      desc: '24K 순금. 개인 이니셜과 후원 일자 각인. 보험 증서에 영구 기재. 5인 한정.',
    },
  ];
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginBottom: 40 }}>
          {marks.map((mk, i) => (
            <button key={i} onClick={() => setMarkIdx(i)} style={{
              background: i === markIdx ? T.goldGlow : 'transparent',
              border: `1px solid ${i === markIdx ? T.goldBorderS : T.border}`,
              color: i === markIdx ? T.gold : T.sub,
              padding: '8px 18px', fontFamily: T.mono, fontSize: 10, letterSpacing: '0.2em', cursor: 'pointer',
            }}>
              {mk.gate}
            </button>
          ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 40, alignItems: 'center' }} className="mark-grid">
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 300 }}>
            <div style={{
              ...
              background: `radial-gradient(circle at 32% 28%, ${T.goldB}, ${T.gold} 40%, ${T.goldDeep} 78%, #3a2f1c)`,
              boxShadow: `0 25px 60px ${T.goldGlow}, inset 0 -20px 40px rgba(0,0,0,0.4), inset 0 15px 30px rgba(255,255,255,0.2)`,
              position: 'relative',
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#3a2f1c' }}>
                <div style={{ fontFamily: T.serif, fontStyle: 'italic', fontSize: 56, fontWeight: 600, lineHeight: 0.9 }}>Aurum</div>
                <div style={{ fontFamily: T.mono, fontSize: 8, letterSpacing: '0.42em', fontWeight: 700, marginTop: 4 }}>999.9 · {markIdx === 0 ? '10K' : '24K'}</div>
                <div style={{ fontFamily: T.mono, fontSize: 7, letterSpacing: '0.3em', marginTop: 18, opacity: 0.6 }}>
                  {markIdx === 0 ? `FNDR·III · #0${29 + Math.floor(Math.random() * 10)}` : `FNDR·V · #00${3 + markIdx}`}
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Mark spec */}
          <div>
            <div style={{ fontFamily: T.mono, fontSize: 9, color: T.goldD, letterSpacing: '0.24em', marginBottom: 10 }}>{m.gate}</div>
    ...
    <section style={{ padding: '100px 20px 120px', background: T.bg, borderTop: `1px solid ${T.border}` }}>
      <div style={{ maxWidth: 720, margin: '0 auto', textAlign: 'center' }}>
        ...
        <button onClick={() => navigate('/signup')} style={{
          ...
          color: T.gold, padding: '18px 44px',
          fontFamily: T.serif, fontStyle: 'italic', fontSize: 20, fontWeight: 400, letterSpacing: '0.03em',
...
function FoundersFooter() {
  return (
    <footer style={{ borderTop: `1px solid ${T.goldBorder}`, padding: '60px 24px 56px', background: T.deepBlack, textAlign: 'center' }}>
      <div style={{ fontFamily: T.serif, fontStyle: 'italic', fontSize: 22, color: T.gold, fontWeight: 500, letterSpacing: '-0.01em', marginBottom: 10 }}>
        Aurum
      </div>
