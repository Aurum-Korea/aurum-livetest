        ...
        <div style={{ position: 'relative', padding: '20px 0 30px' }}>
          ...
          <div className="gate-ladder" style={{ display: 'grid', gridTemplateColumns: `repeat(${GATES.length}, minmax(88px, 1fr))`, gap: 8, position: 'relative' }}>
            {GATES.map((g, i) => {
              ...
              // Apex (Gate III) keeps its gold fill always, but size/glow/scale respond to active state like others
              const bg = g.apex
                ? (active ? T.goldB : T.gold)
                : (active ? T.goldGlow : T.bg);
              const borderColor = g.apex
                ? (active ? '#fff' : T.goldB)
                : (active ? T.goldBorderS : T.goldBorder);
              const textColor = g.apex
                ? T.bg
                : (active ? T.goldB : T.gold);
              return (
                <button key={i} onClick={() => setActiveIdx(i)} style={{
                  ...
                  <div style={{
                    ...
                    boxShadow: active
                      ? (g.apex ? '0 0 32px rgba(232,200,133,0.7)' : '0 0 24px rgba(197,165,114,0.4)')
                      : 'none',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 0 }} className="sim-grid">
            ...
            <div style={{ padding: 30, borderBottom: `1px solid ${T.border}` }}>
              ...
              <div style={{ background: T.deepBlack, border: `1px dashed ${T.border}`, padding: 16, fontFamily: T.mono, fontSize: 11 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10, paddingBottom: 10, borderBottom: `1px dashed ${T.goldBorder}` }}>
                  <span style={{ color: T.gold, letterSpacing: '0.24em', fontSize: 10, fontWeight: 600 }}>CUMULATIVE GMV</span>
                  <span style={{ color: T.text, fontFamily: T.serif, fontStyle: 'italic', fontSize: 26, fontWeight: 500 }}>₩{(sim.totalGMV / 10000).toLocaleString()}만</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                  <span style={{ color: T.muted, letterSpacing: '0.2em', fontSize: 9 }}>PROJECTED @ {(cagr * 100).toFixed(0)}% · {horizon / 12}yr</span>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  <span style={{ color: (sim.projectedValue >= sim.totalGMV) ? (T.green || '#4ade80') : T.red, fontWeight: 600 }}>
                    ...
                    <span style={{ color: T.muted, fontSize: 9, marginLeft: 6 }}>
                      · +₩{Math.max(0, Math.round((sim.projectedValue - sim.totalGMV) / 10000)).toLocaleString()}만
                    </span>
                  </span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 10, borderTop: `1px dashed ${T.goldBorder}` }}>
                  ...
                  <span style={{ color: (T.green || '#4ade80'), fontWeight: 600 }}>
                    ₩{(sim.currentGate ? Math.round(sim.totalGMV * sim.currentGate.d / 100 / 10000) : 0).toLocaleString()}만
                    <span style={{ color: T.muted, fontSize: 9, marginLeft: 6 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 300 }}>
            <div key={markIdx} style={{
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: tone.g4 }}>
                ...
                <div style={{ fontFamily: T.mono, fontSize: 8, letterSpacing: '0.42em', fontWeight: 700, marginTop: 4 }}>{m.purity}</div>
