...
function MarketStrip() {
  ...
  useEffect(() => {
    ...
    series.setData(seedPriceHistory(4842));
    ...
    <Panel num="I" title={ko ? '시장 현황' : 'Market Strip'} sub={ko ? '실시간 · London Fix' : 'Live · London Fix'} live>
      ...
      <div style={{ background: T.bg || '#0a0a0a', border: `1px solid ${T.border}`, padding: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
          <span style={{ fontFamily: T.mono, fontSize: 10, color: T.goldD, letterSpacing: '0.2em' }}>XAU / USD · 30{ko ? '일' : 'D'}</span>
          <span style={{ fontFamily: T.mono, fontSize: 10, color: T.muted }}>area · LBMA ref</span>
        </div>
      ...
      <p style={{ fontFamily: T.serifKr, fontSize: 13, color: T.sub, lineHeight: 1.85, marginTop: 18, maxWidth: 640 }}>
        {ko
          ? '모든 가격은 LBMA 국제 현물가 기준입니다. 실시간 오라클로 30초마다 갱신되며, Aurum에서 거래되는 가격은 여기에 플랫폼 수수료(+8%)가 더해진 값입니다.'
          : 'All prices reference LBMA international spot. Oracle updates every 30 seconds. Aurum transaction price = spot × 1.08 (platform fee). Founders Gate III applies −2% lifetime.'}
      </p>
...
function AccumulatorCalc({ ko, navigate }) {
  ...
  const endValue = totalGrams * spot * 1.05; // modest 5% appreciation baked (pre-launch estimate)
  ...
  return (
    <div>
      ...
      <div style={{ padding: 16, background: T.goldGlow, border: `1px solid ${T.goldBorder}`, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 16 }}>
        ...
        <div>
          <div style={{ fontFamily: T.mono, fontSize: 9, color: T.goldD, letterSpacing: '0.2em', marginBottom: 6 }}>{ko ? '예상 가치' : 'Est. value'}</div>
      ...
      <p style={{ fontFamily: T.sansKr, fontSize: 10, color: T.muted, lineHeight: 1.6, marginTop: 10, textAlign: 'center' }}>
        {ko ? '* 금 가격 연 5% 상승 가정 · 실제 수익률은 시장 상황에 따라 상이' : '* Assumes 5% annual gold appreciation · Actual returns vary'}
      </p>
    </div>
  );
}
...
function Calculators() {
  ...
  const ko = lang === 'ko';
  const navigate = useNavigate();
  const [tab, setTab] = useState(2); // default to Accumulator
  const tabs = [
  ...
  return (
    <Panel num="VII" title={ko ? '계산기' : 'Calculators'} sub={ko ? '3 tools · 당신의 숫자' : 'Your numbers, 3 tools'}>
      ...
      {tab === 0 && <KumtongConverter ko={ko} />}
      {tab === 1 && <OverpayCalc ko={ko} />}
      {tab === 2 && <AccumulatorCalc ko={ko} navigate={navigate} />}
    </Panel>
  );
}
