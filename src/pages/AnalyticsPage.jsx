...
function MarketStrip() {
  ...
  const [metric, setMetric] = useState('gold');    // gold | silver | gs | gk
  const [timeframe, setTimeframe] = useState('1M'); // 1M|3M|6M|1Y|5Y|Max
  ...
  // Metric + timeframe definitions
  const METRICS = {
    gold:   { lblKo: '금 가격',       lblEn: 'Gold · XAU/USD',      base: 4842, vol: 0.006, unit: '$',  color: '#C5A572' },
    silver: { lblKo: '은 가격',       lblEn: 'Silver · XAG/USD',    base: 56.4, vol: 0.010, unit: '$',  color: '#d4d4d8' },
    gs:     { lblKo: '금/은 비율',    lblEn: 'Gold · Silver ratio', base: 85.9, vol: 0.008, unit: 'x',  color: '#9ca3af' },
    gk:     { lblKo: '금/코스피',     lblEn: 'Gold · KOSPI ratio',  base: 1.89, vol: 0.009, unit: 'x',  color: '#fbbf24' },
  };
  const TIMEFRAMES = {
    '1M':  { days: 30,   label: '1M' },
    '3M':  { days: 90,   label: '3M' },
  ...
  // Redraw on metric/timeframe change
  useEffect(() => {
    ...
    const series = chartRef.current.addAreaSeries({
      lineColor: m.color,
      topColor: m.color + '3a',  // 22% alpha
      bottomColor: m.color + '05',
      lineWidth: 2,
      priceLineVisible: false,
      crosshairMarkerVisible: true,
    });
    // Sample points from the requested window — keep array manageable
    const samplePoints = Math.min(tf.days, 400);
    const stride = Math.max(1, Math.floor(tf.days / samplePoints));
    const data = [];
    let p = m.base * 0.88;
    const now = Date.now();
    for (let i = tf.days; i >= 0; i -= stride) {
      p += (Math.random() - 0.42) * m.base * m.vol;
      p = Math.max(p, m.base * 0.78);
      const d = new Date(now - i * 86400000);
      data.push({
        time: `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`,
        value: parseFloat(p.toFixed(4)),
      });
    }
    series.setData(data);
  ...
  return (
    <Panel num="I" title={ko ? '시장 현황' : 'Market Strip'} sub={ko ? '실시간 · London Fix' : 'Live · London Fix'} live>
      {/* Top stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }} className="analytics-grid-4">
        {[
          { k: 'XAU / USD', v: fmt.usd(spot), sub: `per oz · ${fmt.usd(spot / OZ_G)} /g`, tone: '+0.78%', pos: true },
          { k: 'XAU / KRW', v: fmt.krw(krwPerG), sub: ko ? '1g 기준' : 'per gram', tone: '+1.22%', pos: true },
          { k: 'USD / KRW', v: fmt.n(krw, 2), sub: ko ? '원화 환율' : 'FX spot', tone: '+0.31%', pos: true },
          { k: 'KR PREM', v: '20.1%', sub: ko ? '한국 소매 프리미엄' : 'KR retail premium', tone: '+0.4bp', pos: true },
        ].map((r, i) => (
          <div key={i} style={{ padding: '14px 14px', background: T.bg || '#0a0a0a', border: `1px solid ${T.border}` }}>
            <div style={{ fontFamily: T.mono, fontSize: 9, color: T.goldD, letterSpacing: '0.22em', marginBottom: 6 }}>{r.k}</div>
            <div style={{ fontFamily: T.mono, fontSize: 19, color: T.text, fontWeight: 600 }}>{r.v}</div>
      ...
      {/* Chart container with metric + timeframe toggles */}
      <div style={{ background: T.bg || '#0a0a0a', border: `1px solid ${T.border}`, padding: 16 }}>
        {/* Toggle rows */}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            {Object.keys(TIMEFRAMES).map(key => {
              ...
              return (
                <button key={key} onClick={() => setTimeframe(key)} type="button" style={{
                  padding: '6px 10px',
                  background: active ? T.goldGlow : 'transparent',
                  border: `1px solid ${active ? T.gold : T.border}`,
                  color: active ? T.gold : T.sub,
                  fontFamily: T.mono, fontSize: 10, fontWeight: active ? 700 : 500, letterSpacing: '0.1em',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
          <span style={{ fontFamily: T.mono, fontSize: 10, color: T.goldD, letterSpacing: '0.2em' }}>
            {ko ? activeMetric.lblKo : activeMetric.lblEn} · {timeframe}
          </span>
          <span style={{ fontFamily: T.mono, fontSize: 10, color: T.muted }}>
            {metric === 'gold' || metric === 'silver' ? 'LBMA ref' : 'ratio'}
          </span>
        </div>
        <div ref={containerRef} style={{ width: '100%' }} />
      </div>

      <p style={{ fontFamily: T.serifKr, fontSize: 13, color: T.sub, lineHeight: 1.85, marginTop: 18, maxWidth: 640 }}>
        {ko
          ? '모든 가격은 LBMA 국제 현물가 기준입니다. 실시간 오라클로 30초마다 갱신되며, Aurum에서 거래되는 가격은 여기에 플랫폼 수수료(+8%)가 더해진 값입니다. 지표와 기간을 전환해 장기 맥락을 확인하세요.'
          : 'All prices reference LBMA international spot. Oracle updates every 30 seconds. Aurum transaction price = spot × 1.08 (platform fee). Toggle metrics and timeframes to see long-term context.'}
      </p>
...
function KimchiMeter() {
  ...
  return (
    <Panel num="II" title={ko ? '김치 프리미엄 지표' : 'Kimchi Premium Meter'} sub={ko ? '국내 소매 vs LBMA' : 'KR retail vs LBMA'} live>
      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 36, alignItems: 'center' }} className="kimchi-grid">
        {/* Gauge */}
        <div style={{ position: 'relative', width: 200, height: 140 }}>
          ...
          <div style={{ position: 'absolute', top: 60, left: '50%', transform: 'translateX(-50%)', textAlign: 'center' }}>
            <div style={{ fontFamily: T.mono, fontSize: 26, fontWeight: 700, color: zc }}>{prem.toFixed(1)}%</div>
            <div style={{ fontFamily: T.sans, fontSize: 11, color: zc, marginTop: 2 }}>{zl}</div>
          </div>
        </div>
        ...
        <div>
          ...
          <div>
            {[
              ...
              <div key={i} style={{
                ...
                alignItems: 'center',
                padding: '10px 0',
                borderBottom: i < 2 ? '1px solid rgba(197,165,114,0.06)' : 'none',
                background: r.me ? 'rgba(74,222,128,0.04)' : 'transparent',
                paddingLeft: r.me ? 8 : 0,
                marginLeft: r.me ? -8 : 0,
                marginRight: r.me ? -8 : 0,
                paddingRight: r.me ? 8 : 0,
              }}>
          ...
          <div style={{ marginTop: 16, padding: '14px 16px', background: T.goldGlow, border: `1px solid ${T.goldBorder}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <div>
              <div style={{ fontFamily: T.mono, fontSize: 9, color: T.goldD, letterSpacing: '0.22em', marginBottom: 4 }}>
...
function Ratios() {
  ...
  return (
    <Panel num="III" title={ko ? '5대 비율' : 'Five Ratios'} sub={ko ? '탭 전환 · 장기 맥락' : 'Long-term context'}>
      {/* Tab bar */}
...
function KRWDebase() {
  ...
  return (
    <Panel num="IV" title={ko ? 'KRW 구매력 감소' : 'KRW Debasement'} sub={ko ? '₩1M 기준 · 금 그램으로 환산' : '₩1M in grams of gold'}>
...
function CBBuying() {
  ...
  return (
    <Panel num="V" title={ko ? '중앙은행 금 매입 흐름' : 'Central Bank Buying Cadence'} sub={ko ? '2010 → 2025 · 연간 매입량' : 'Annual net purchases'}>
...
function KoreaAllocation() {
  ...
  const ko = lang === 'ko';
  return (
    <Panel num="VI" title={ko ? '한국 가계 vs 중앙은행 자산 배분' : 'KR Household vs CB Reserves'} sub={ko ? '당신은 어디에 서 있습니까' : 'Where do you stand'}>
  ...
  return (
    <div>
      ...
      <input type="range" min="1" max="100" step="1" value={grams} onChange={e => setGrams(+e.target.value)}
        style={{ width: '100%', accentColor: T.gold, marginBottom: 20 }} />
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginBottom: 18 }}>
        <div>
          ...
          <input type="range" min="200000" max="5000000" step="100000" value={monthly} onChange={e => setMonthly(+e.target.value)} style={{ width: '100%', accentColor: T.gold }} />
        </div>
        <div>
          ...
          <input type="range" min="1" max="25" step="1" value={years} onChange={e => setYears(+e.target.value)} style={{ width: '100%', accentColor: T.gold }} />
        </div>
      </div>
...
function Calculators() {
  ...
  return (
    <Panel num="VII" title={ko ? '계산기' : 'Calculators'} sub={ko ? '3 tools · 당신의 숫자' : 'Your numbers, 3 tools'}>
      <div style={{ display: 'flex', borderBottom: `1px solid ${T.border}`, marginBottom: 22, overflowX: 'auto' }}>
        {tabs.map((t, i) => (
          <button key={i} onClick={() => setTab(i)} style={{
            background: tab === i ? 'rgba(197,165,114,0.1)' : 'transparent',
...
function PriceLane() {
  ...
  return (
    <Panel num="VIII" title={ko ? 'Aurum 가격 체계' : 'Aurum Price Lane'} sub={ko ? '숨김 없는 4 라인 비교' : 'Transparent 4-lane comparison'} live>
