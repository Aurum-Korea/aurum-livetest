...
/* ═══════════════════════════════════════════════════════════════════════
   WHY GOLD PAGE
   ═══════════════════════════════════════════════════════════════════════ */
export function WhyGoldPage({ lang, navigate, prices, krwRate }) {
  const ko = lang === 'ko';
    ...
    <div style={{ background: T.bg }}>
      ...
      <div style={{ position:'relative', overflow:'hidden', background:'linear-gradient(135deg, #0a0a0a 0%, #111008 50%, #0a0a0a 100%)', borderBottom:`1px solid ${T.goldBorder}` }}>
        ...
        <div className="aurum-container" style={{ paddingTop: isMobile ? 48 : 100, paddingBottom: isMobile ? 48 : 100, position:'relative', zIndex:1 }}>
          ...
          <p style={{ fontFamily:T.sans, fontSize: isMobile ? 15 : 16, color:T.textSub, lineHeight:1.85, maxWidth:560, marginBottom:36 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns: isMobile?'1fr':'repeat(3,1fr)', gap:2, background:T.border }}>
            {WHY_GOLD_REASONS.map((r,i) => (
              <div key={i} style={{ background:'#0d0b08', padding: isMobile?'24px 20px':'32px 28px', position:'relative', overflow:'hidden' }}>
                ...
                <p style={{ fontFamily:T.sans, fontSize:14, color:T.textSub, lineHeight:1.8, marginBottom:16 }}>{r.body}</p>
                  ...
                  {[
                    ...
                    return (
                      <tr key={ri} style={{ background: ri % 2 === 0 ? T.bg : T.bg2 }}
                        ...
                        <td style={{ padding: isMobile ? '10px 10px' : '13px 18px', fontFamily: T.sans, fontSize: isMobile ? 13 : 14, color: T.text, borderBottom: `1px solid ${T.border}` }}>{label}</td>
                        {vals.map((val, ci) => (
          ...
          <div style={{ borderBottom: `1px solid ${T.border}` }}>
            <div className="aurum-container" style={{ paddingTop: isMobile ? 28 : 64, paddingBottom: isMobile ? 28 : 56 }}>
              ...
              <p style={{ fontFamily: T.sans, fontSize: 11, color: T.textMuted, marginTop: 12, lineHeight: 1.9, textAlign: 'center' }}>
                * {ko ? '실물 금고 외반출 시 별도 수수료 발생. 100g 미만 GoldPath 전환은 무료.' : 'Fee applies when physical metal leaves the vault. GoldPath conversion at 100g+ is free.'}
              </p>
            </div>
          </div>
        );
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns: isMobile?'1fr':'1fr 1fr', gap:16 }}>
      ...
      <div style={{ background:T.bgCard, border:`1px solid ${T.goldBorder}`, padding: isMobile?'20px 16px':'28px' }}>
        <div style={{ fontFamily:T.mono, fontSize:10, color:T.goldDim, letterSpacing:'0.22em', textTransform:'uppercase', marginBottom:16 }}>GoldPath LAUNCH ESTIMATOR</div>
      ...
      <div style={{ background:T.bgCard, border:`1px solid ${T.goldBorder}`, padding: isMobile?'20px 16px':'28px' }}>
        ...
        <p style={{ fontFamily:T.sansKr, fontSize:14, color:T.textSub, lineHeight:1.9, marginBottom:20 }}>
        ...
        <div style={{ marginTop:16 }}>
          <button onClick={() => navigate('agp-intro')} style={{ width:'100%', background:'rgba(197,165,114,0.06)', border:`1px solid rgba(197,165,114,0.3)`, color:'#c5a572', padding:'14px 28px', cursor:'pointer', fontFamily:T.sans, fontSize:13 }}>
            {ko?'GoldPath 금환 시작하기 →':'Start GoldPath →'}
          </button>
        </div>
      </div>
    </div>
...
export function AGPPage({ lang, navigate, currency = 'KRW', krwRate = 1368, prices = null }) {
  ...
  const faqItems = [
    ...
    { icon: <IB><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg></IB>, title: ko ? '내 그램은 안전한가요?' : 'Is my gold safe?', content: ko ? "모든 GoldPath 그램은 실제 금속으로 100% 백킹됩니다. 매일 감사 리포트가 공개되며 Lloyd's of London 보험이 전액 적용됩니다." : "All GoldPath grams are 100% backed by real metal. Daily audit reports published. Full Lloyd's of London insurance." },
  ...
  return (
    <div style={{ background: T.bg }}>
        ...
        <div className="aurum-container" style={{ paddingTop: isMobile ? 32 : 80, paddingBottom: isMobile ? 32 : 72, display: isMobile ? 'block' : 'flex', alignItems: 'center', gap: 48 }}>
          <div style={{ maxWidth: 680, flex: 1 }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <GoldPathMark scale={isMobile ? 0.75 : 1.0} />
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, flexDirection: isMobile ? 'column' : 'row', alignItems: 'stretch' }}>
              <button onClick={() => navigate('agp-intro')} className="btn-primary" style={{ flex: 1, minWidth: 180 }}>{ko ? 'GoldPath 시작하기' : 'Start GoldPath'}</button>
      ...
      <div style={{ borderBottom: `1px solid ${T.goldBorder}`, background: `linear-gradient(180deg, ${T.goldGlow}, ${T.bg})` }}>
        <div className="aurum-container" style={{ paddingTop: isMobile ? 28 : 64, paddingBottom: isMobile ? 28 : 64 }}>
          ...
          <div style={{ marginTop: 20, textAlign: 'center' }}>
            <button onClick={() => navigate('campaign-agp-launch')} style={{ background: T.gold, border: 'none', color: '#0a0a0a', padding: '14px 28px', fontFamily: T.sans, fontSize: 14, fontWeight: 700, letterSpacing: '0.04em', cursor: 'pointer' }}>
              {ko ? 'GoldPath 론치 보기 →' : 'View GoldPath Launch →'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: T.bg1, borderBottom: `1px solid ${T.border}` }}>
        <div className="aurum-container" style={{ paddingTop: isMobile ? 28 : 64, paddingBottom: isMobile ? 28 : 64 }}>
          <div style={{ maxWidth: 760, margin: '0 auto' }}>
            <SectionHead badge="자주 묻는 질문" title="GoldPath FAQ" align="left" />
            <Accordion items={faqItems} />
          </div>
        </div>
      </div>
    ...
    <div style={{ background: T.bg, minHeight: '80vh' }}>
      <div className="aurum-container" style={{ paddingTop: isMobile ? 28 : 60, paddingBottom: isMobile ? 28 : 72 }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <div style={{ fontFamily: T.mono, fontSize: 11, color: T.goldDim, letterSpacing: '0.16em', marginBottom: 10, textTransform: 'uppercase' }}>GoldPath · 일일 백킹 리포트</div>
          ...
          <div style={{ background: 'rgba(74,222,128,0.06)', border: '1px solid rgba(74,222,128,0.25)', padding: '20px 24px', marginBottom: 28, display: 'flex', alignItems: 'center', gap: 16, position: 'relative', overflow: 'hidden' }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: T.sans, fontSize: 12, color: T.textMuted }}>모든 GoldPath 그램은 실물 금속으로 100% 뒷받침됩니다.</div>
            </div>
          </div>

          {[
            { label: ko ? '총 GoldPath 그램 발행량' : 'Total GoldPath Grams Issued', value: '4,218.76 g', sub: '135.63 oz' },
          ...
          <div style={{ marginTop: 24, display: 'flex', gap: 12 }}>
            <button onClick={() => navigate('agp')} className="btn-primary">{ko ? 'GoldPath 가입하기' : 'Join GoldPath'}</button>
...
// ── GoldTodayCharts — price charts with 1Y/2Y/5Y/All range toggles (Mkt·3) ──
function GoldTodayCharts({ prices, krwRate, lang, isMobile, ko, MONO, SANS }) {
  const [range, setRange] = useState('1Y');
  const RANGES = ['1Y', '2Y', '5Y', 'All'];
  const daysMap = { '1Y': 365, '2Y': 730, '5Y': 1825, 'All': 3650 };
  const goldUSD   = prices?.gold   || 3342;
  const silverUSD = prices?.silver || 32.9;
  return (
    <div>
      {/* Header row with range toggles */}
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        <div style={{ fontFamily:MONO, fontSize:10, color:'rgba(197,165,114,0.5)', letterSpacing:'0.2em', textTransform:'uppercase' }}>
          {ko ? '실시간 가격 차트' : 'Live Price Charts'}
        </div>
        <div style={{ display:'flex', gap:2 }}>
          {RANGES.map(r => (
            <button key={r} onClick={() => setRange(r)} style={{
              background: range === r ? 'rgba(197,165,114,0.15)' : 'transparent',
              border: `1px solid ${range === r ? 'rgba(197,165,114,0.6)' : 'rgba(197,165,114,0.15)'}`,
              color: range === r ? '#C5A572' : '#555',
      ...
      <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr', gap:isMobile?16:24 }}>
        <div style={{ background:'rgba(197,165,114,0.03)', border:'1px solid rgba(197,165,114,0.1)', padding:'20px 20px 12px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:12 }}>
            <span style={{ fontFamily:MONO, fontSize:11, color:'#c5a572', letterSpacing:'0.12em', textTransform:'uppercase' }}>{ko?'금 XAU/USD':'Gold XAU/USD'}</span>
            <span style={{ fontFamily:MONO, fontSize:18, color:'#E3C187', fontWeight:700 }}>${goldUSD.toFixed(2)}</span>
          </div>
          <PriceChart price={goldUSD} metal="gold" days={daysMap[range]} height={isMobile?140:200} />
        </div>
        <div style={{ background:'rgba(125,211,220,0.03)', border:'1px solid rgba(125,211,220,0.1)', padding:'20px 20px 12px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:12 }}>
            <span style={{ fontFamily:MONO, fontSize:11, color:'#7dd3dc', letterSpacing:'0.12em', textTransform:'uppercase' }}>{ko?'은 XAG/USD':'Silver XAG/USD'}</span>
            <span style={{ fontFamily:MONO, fontSize:18, color:'#7dd3dc', fontWeight:700 }}>${silverUSD.toFixed(2)}</span>
          </div>
          <PriceChart price={silverUSD} metal="silver" days={daysMap[range]} height={isMobile?140:200} />
        </div>
      </div>
      <div style={{ fontFamily:MONO, fontSize:9, color:'#333', marginTop:10 }}>
        {ko ? `* 30일 ~ 10년 시뮬레이션 데이터 · 현재 가격 실시간 업데이트` : `* Simulated price history scaled to current live price · Updates in real time`}
      </div>
    </div>
  );
}
...
export function GoldTodayPage({ lang, navigate, prices, krwRate = 1440 }) {
  ...
  return (
    <div style={{ background:T_loc.bg, minHeight:'100vh' }}>
      ...
      {/* §5 Live Price Charts — 1Y/2Y/5Y/All with range toggles (Mkt·3) */}
      <div style={{ background:'#0a0908', borderBottom:'1px solid rgba(197,165,114,0.08)', padding:isMobile?'40px 20px':'64px 0' }}>
        <div className="aurum-container">
          <GoldTodayCharts prices={prices} krwRate={krwRate} lang={lang} isMobile={isMobile} ko={ko} MONO={MONO} SANS={SANS} />
        </div>
      </div>

      {/* §6 Market Ratios — CB Holdings / Gold-Silver / Gold-Dow / Real Estate */}
      <div style={{ borderBottom:'1px solid rgba(197,165,114,0.08)', padding:isMobile?'40px 20px':'64px 0' }}>
        <div className="aurum-container">
          <div style={{ fontFamily:MONO, fontSize:10, color:'rgba(197,165,114,0.5)', letterSpacing:'0.2em', textTransform:'uppercase', marginBottom:8 }}>
            {ko?'시장 분석 도구 · 4가지 비율':'Market Ratios · 4 Key Indicators'}
          </div>
          <h2 style={{ fontFamily:ko?T_loc.serifKr:SERIF, fontStyle:'italic', fontSize:isMobile?22:32, fontWeight:300, color:'#f5f0e8', margin:'0 0 28px' }}>
            {ko?<>숫자가 말합니다 — <span style={{ color:'#c5a572' }}>지금이 보유 시점입니다</span></> : <>The data speaks — <span style={{ color:'#c5a572', fontStyle:'italic' }}>the case to hold now</span></>}
          </h2>
          <MarketRatios lang={lang} prices={prices} krwRate={krwRate || 1440} />
        </div>
      </div>
      ...
      <div style={{ background:'#0d0b08', padding:isMobile?'48px 20px':'80px 0', textAlign:'center' }}>
        <div className="aurum-container" style={{ maxWidth:560 }}>
          <div style={{ fontFamily:ko?T_loc.serifKr:SERIF, fontStyle:'italic', fontSize:isMobile?22:32, color:'#f5f0e8', marginBottom:12, lineHeight:1.2 }}>
            {ko?<>GoldPath로 이 격차를 <span style={{ color:'#c5a572' }}>피하는 방법 →</span></>:'Avoid this gap with GoldPath →'}
          </div>
