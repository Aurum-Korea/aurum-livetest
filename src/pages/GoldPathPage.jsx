...
// AURUM · /goldpath
// 
// Mission: close the monthly subscription. Every CTA → /signup.
// Founders Club appears once in FAQ as an automatic benefit, never linked.
// V4 Quiet polish: editorial breath, no aggressive scarcity, tier-driven.
// ═══════════════════════════════════════════════════════════════════════

// Tier data · user-locked source of truth
const TIERS = [
  { num:'I',   nameKR:'브론즈',   nameEN:'Bronze · the opening',     monthly:'₩200,000+',   min:200000,   gift:50000,    featured:false,
    perks:['Founding Year 가격 12개월','스테인리스 마크 발급','Founders Club 자동 등록'] },
  { num:'II',  nameKR:'실버',     nameEN:'Silver · the accelerator', monthly:'₩500,000+',   min:500000,   gift:150000,   featured:false,
    perks:['Founding 500 가격 24개월','시리얼 번호 #001–500','Vault Weekend 초대'] },
  { num:'III', nameKR:'골드',     nameEN:'Gold · the prestige',      monthly:'₩1,000,000+', min:1000000,  gift:400000,   featured:true,
    perks:['Founder 우선가 12개월','골드 마크 예약','Concierge 전화 연결'] },
  { num:'IV',  nameKR:'플래티넘', nameEN:'Platinum · the patron',    monthly:'₩2,000,000+', min:2000000,  gift:1500000,  featured:false,
    perks:['Patron 최우선가 평생','10K 솔리드 골드 마크','SG Vault 단독 방문'] },
  { num:'V',   nameKR:'소브린',   nameEN:'Sovereign · the apex',     monthly:'₩5,000,000+', min:5000000,  gift:5000000,  featured:false,
...
// ─── GoldPath coin mark (inline SVG · line → diamond + serif italic + 금환) ───
function GoldPathCoin({ size = 320 }) {
  ...
  return (
    <svg viewBox="0 0 220 64" width={size} height={h} style={{ display: 'block' }}>
      ...
      <text x="18" y="25" textAnchor="middle" fontFamily="'Cormorant Garamond',Georgia,serif" fontStyle="italic" fontWeight="500" fontSize="11" fill="rgba(40,26,12,0.82)" letterSpacing="-0.02em">A</text>
...
function HeroCalc() {
  ...
  return (
    <div style={{
      ...
      {/* Result tile */}
      <div style={{ padding: '14px 14px', background: T.goldGlow || 'rgba(197,165,114,0.08)', border: `1px solid ${T.goldBorder}`, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        <div>
...
function Hero() {
  ...
  return (
    <section style={{
      ...
      background: `radial-gradient(ellipse at 75% 30%,rgba(197,165,114,0.10),transparent 55%), linear-gradient(180deg, ${T.bg} 0%, ${T.bg1 || T.bg} 100%)`,
      borderBottom: `1px solid ${T.goldBorder}`,
      position: 'relative', overflow: 'hidden',
    }}>
      <div style={{ maxWidth: 1180, margin: '0 auto', display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 64, alignItems: 'center' }} className="gp-hero-grid">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
            <div style={{ width: 32, height: 1, background: T.gold }} />
            <span style={{ fontFamily: T.serif, fontStyle: 'italic', fontSize: 13, color: T.gold, letterSpacing: '0.04em' }}>GoldPath · Founding Year</span>
            <span style={{ color: T.goldD, fontSize: 11 }}>·</span>
            <span style={{ fontFamily: T.mono, fontSize: 11, color: T.gold, letterSpacing: '0.18em', textTransform: 'uppercase' }}>창립 시즌</span>
          </div>
          <h1 style={{
            fontFamily: T.serifKr, fontWeight: 500,
            fontSize: 'clamp(32px, 4.6vw, 52px)',
            lineHeight: 1.14, color: T.text,
            margin: '0 0 18px', letterSpacing: '-0.01em',
          }}>
            매달 한 그램.
            <br />
            <span style={{ color: T.gold, fontFamily: T.serif, fontStyle: 'italic' }}>평생 쌓입니다.</span>
          </h1>
          <div style={{ fontFamily: T.serif, fontStyle: 'italic', fontSize: 17, color: T.goldD, marginBottom: 22, lineHeight: 1.45 }}>
            One gram a month. A lifetime of gold.
          </div>
          <p style={{ fontFamily: T.serifKr, fontSize: 16, color: T.sub, lineHeight: 1.85, fontWeight: 300, marginBottom: 34, maxWidth: 520 }}>
            매월 자동이체로 실물 금이 싱가포르 FTZ 금고에 회원님 이름으로 적립됩니다. 첫 결제일, 선택한 티어에 따라 <strong style={{ color: T.text }}>창립 기프트가 실물 금으로 즉시 추가</strong>됩니다.
          </p>
          <div style={{ display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
            <PrimaryCTA to="/signup">GoldPath 가입하기 →</PrimaryCTA>
            <GhostCTA onClick={() => {
              const el = document.getElementById('gp-calc');
              if (el) el.scrollIntoView({ behavior: 'smooth' });
            }}>티어별 기프트 보기</GhostCTA>
          </div>
        </div>

        {/* Right: MZ calculator + coin mark below */}
        <div className="gp-hero-visual" style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', gap: 18 }}>
          <HeroCalc />
          <div style={{
      ...
      <style>{`
        @media (max-width: 900px) {
          .gp-hero-grid { grid-template-columns: 1fr !important; gap: 40px !important; }
...
function Stats() {
  const rows = [
    { v: '5', l: '론치 기프트 티어' },
    { v: '₩50K–₩5M', l: '티어별 실물 금 기프트' },
    { v: '30일', l: '잠금 해제 후 영구 소유' },
    { v: 'GMV+', l: '성장할수록 추가 크레딧' },
  ];
  return (
    <div style={{ background: T.bg1 || '#0d0b08', borderBottom: `1px solid ${T.border}` }}>
      <div style={{ maxWidth: 1180, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)' }} className="gp-stats">
        {rows.map((s, i) => (
          <div key={i} style={{
            textAlign: 'center', padding: '22px 16px',
            borderRight: i < 3 ? `1px solid ${T.border}` : 'none',
          }}>
            <div style={{ fontFamily: T.mono, fontSize: 20, color: T.gold, fontWeight: 700 }}>{s.v}</div>
            <div style={{ fontFamily: T.sansKr, fontSize: 12, color: T.muted, marginTop: 5 }}>{s.l}</div>
          </div>
        ))}
      </div>
      <style>{`
        @media (max-width: 640px) {
          ...
          .gp-stats > div:nth-child(1), .gp-stats > div:nth-child(2) { border-bottom: 1px solid ${T.border}; }
          .gp-stats > div:nth-child(2) { border-right: none !important; }
        }
      `}</style>
    </div>
  );
...
function HowItWorks() {
  const steps = [
    { roman: 'I',   kr: '예약',     desc: '이메일·휴대폰으로 자리 확보. 론치 시 자동 안내.' },
    { roman: 'II',  kr: '첫 결제',  desc: '토스·카카오·은행 자동이체로 첫 적립.' },
    { roman: 'III', kr: '기프트 적립', desc: '결제 즉시 선택 티어 창립 기프트가 실물 금으로 계정에 적립.' },
  ];
  return (
    <section style={{ padding: '72px 24px', background: T.bg, borderBottom: `1px solid ${T.border}` }}>
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          <div style={{ fontFamily: T.mono, fontSize: 10, color: T.gold, letterSpacing: '0.3em', marginBottom: 12, textTransform: 'uppercase' }}>§ I · How it works</div>
          <h2 style={{ fontFamily: T.serifKr, fontSize: 'clamp(24px, 3vw, 34px)', fontWeight: 500, color: T.text, margin: 0, lineHeight: 1.2 }}>
            가입부터 첫 적립까지, <span style={{ fontFamily: T.serif, fontStyle: 'italic', color: T.gold }}>세 걸음</span>
          </h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 32 }} className="gp-how">
          {steps.map((s, i) => (
            <div key={i} style={{ textAlign: 'center', padding: '0 10px' }}>
              <div style={{
                display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                width: 56, height: 56,
                border: `1px solid ${T.goldBorder}`, borderRadius: '50%',
                marginBottom: 18,
                fontFamily: T.serif, fontStyle: 'italic', fontSize: 20, color: T.gold, fontWeight: 500,
                background: T.bg,
              }}>{s.roman}</div>
              <div style={{ fontFamily: T.serifKr, fontSize: 18, fontWeight: 500, color: T.text, marginBottom: 8 }}>{s.kr}</div>
              <div style={{ fontFamily: T.sansKr, fontSize: 13, color: T.sub, lineHeight: 1.8, maxWidth: 220, margin: '0 auto' }}>{s.desc}</div>
            </div>
          ))}
        </div>
      </div>
      <style>{`
        @media (max-width: 720px) {
          .gp-how { grid-template-columns: 1fr !important; gap: 28px !important; }
        }
      `}</style>
    </section>
  );
          ...
          {TIERS.map((tier, i) => {
            ...
            return (
              <div key={i} onClick={() => setActiveTier(i)} style={{
                ...
                boxShadow: isActive ? `0 10px 32px rgba(197,165,114,0.14)` : 'none',
  ...
  return (
    <div style={{ padding: '28px 24px', background: T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 22, borderBottom: `1px solid ${T.border}` }}>
      ...
      <div style={{
        ...
        boxShadow: `0 0 16px rgba(197,165,114,0.15)`,
        flexShrink: 0,
      }}>
...
function FAQ() {
  ...
  const items = [
    { q: '창립 기프트는 현금입니까, 금입니까?', a: '금입니다. 적립 시점의 국제 시세 기준 해당 금액의 그램이 회원님 계정에 즉시 적립됩니다. 싱가포르 Malca-Amit FTZ 금고에 실물로 배분 보관됩니다.' },
    { q: '그로스 크레딧은 언제 지급되나요?', a: 'GoldPath 누적 GMV가 해당 게이트 문턱을 초과하는 시점에 자동 지급됩니다. 내 구매 + 추천인 누적 합산으로 카운트됩니다.' },
    { q: '30일 내 해지하면 기프트는 어떻게 되나요?', a: '창립 기프트는 첫 결제일 기준 30일간 잠금됩니다. 30일 이내 해지 시 기프트 그램은 자동 회수되고 원금은 전액 환급. 30일 이후에는 완전한 회원님 자산입니다.' },
    { q: 'GoldPath와 Founders Club의 관계는?', a: 'GoldPath 가입 시 Founders Club에 자동 등록됩니다. 월 적립액이 GMV로 카운트되어 5개 게이트를 향해 누적. 두 혜택이 동시 적용됩니다 — 별도 가입은 필요하지 않습니다.' },
  ];
  ...
  return (
    <section style={{ padding: '80px 24px', borderBottom: `1px solid ${T.border}` }}>
      <div style={{ maxWidth: 820, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 44 }}>
          <div style={{ fontFamily: T.mono, fontSize: 10, color: T.gold, letterSpacing: '0.3em', marginBottom: 12, textTransform: 'uppercase' }}>§ VI · FAQ</div>
          <h2 style={{ fontFamily: T.serifKr, fontSize: 'clamp(24px, 3vw, 34px)', fontWeight: 500, color: T.text, margin: '0 0 10px', lineHeight: 1.2 }}>
            자주 묻는 <span style={{ fontFamily: T.serif, fontStyle: 'italic', color: T.gold }}>질문.</span>
          </h2>
          <div style={{ fontFamily: T.serif, fontStyle: 'italic', fontSize: 14, color: T.goldD }}>Quietly answered.</div>
        </div>
        <div>
          {items.map((item, i) => {
            const isOpen = open === i;
            return (
              <div key={i} style={{ borderTop: i === 0 ? `1px solid ${T.border}` : 'none', borderBottom: `1px solid ${T.border}` }}>
                <button onClick={() => setOpen(isOpen ? null : i)} style={{
                  width: '100%', padding: '22px 4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16,
                  background: 'transparent', border: 'none', cursor: 'pointer',
                  fontFamily: T.serifKr, fontSize: 16, fontWeight: 500, color: T.text, textAlign: 'left',
                }}>
                  {item.q}
                  <span style={{ fontFamily: T.mono, fontSize: 18, color: T.gold, flexShrink: 0, transform: isOpen ? 'rotate(45deg)' : 'none', transition: 'transform 0.2s' }}>+</span>
                </button>
                {isOpen && (
                  <div style={{ padding: '0 4px 22px', fontFamily: T.serifKr, fontSize: 14, color: T.sub, lineHeight: 1.9, fontWeight: 300 }}>
                    {item.a}
                  </div>
                )}
              </div>
...
function FinalCTA() {
  return (
    <section style={{ padding: '80px 24px', textAlign: 'center', background: `radial-gradient(ellipse at 50% 30%, rgba(197,165,114,0.10), transparent 60%), ${T.bg}` }}>
...
export default function GoldPathPage() {
  const [activeTier, setActiveTier] = useState(2); // Gold = featured default

  return (
    <div style={{ background: T.bg, color: T.text, minHeight: '100vh' }}>
      <TickerBar />
      <QuietNav page="goldpath" />
