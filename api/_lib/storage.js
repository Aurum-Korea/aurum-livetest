...
//
// Falls back to an in-memory store ONLY for local dev with no env set;
// in production with no env set, every call throws so misconfig is loud.
...
async function call(cmd) {
  // cmd = ["SET","key","val"] etc.  POST to /pipeline with [[cmd]] for atomicity, or just /
  const url = URL_();
  const tok = TOK_();
  if (!url || !tok) {
    if (isProd()) throw new Error('Upstash KV not configured (KV_REST_API_URL / KV_REST_API_TOKEN)');
    // Local-dev fallback — minimal in-memory adapter for the few commands we use.
    return memCall(cmd);
  }
