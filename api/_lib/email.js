...
export function buildInvitationEmail({ lead, code, accessUrl }) {
  ...
  const text = [
    ...
    ``,
    `The code below opens the materials. Review them at your pace, then proceed to the confidentiality agreement when ready.`,
    ``,
    `Access code:  ${code}`,
    ...
    ``,
    `아래 코드로 자료를 열람하실 수 있습니다. 충분히 검토하신 후, 준비가 되시면 비밀유지 계약 단계로 진행해 주십시오.`,
    ``,
    `접근 코드:  ${code}`,
    `자료 열기:   ${accessUrl}`,
    ``,
  ...
  const inner = `
    ...
    <tr><td bgcolor="#0a0a0a" align="left" style="background:#0a0a0a;padding:0 32px 30px">
      <a href="${accessUrl}" style="display:inline-block;padding:13px 22px;background:#C5A572;color:#0a0a0a;text-decoration:none;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:10.5px;letter-spacing:.22em;font-weight:600;white-space:nowrap">OPEN MATERIALS &nbsp;→</a>
    </td></tr>
    ...
    ${signOffRow}
  `;
  const html = shellHtml({ inner, hidden_preheader: 'The code below opens the materials.' });
  return { subject, text, html };
}
