...
export function buildMagicLinkEmail({ lead, linkUrl }) {
  const subject = `Sign in to your AURUM portfolio  ·  `;
  const text = [
    `Sign in to your AURUM portfolio.This link logs you in directly. It expires in 15 minutesand works once.`,
    linkUrl,
    `If you didn't request this, ignore this email.— The PartnersAurum · TACC Pte Ltd · Singapore─────────────────────────────────────────AURUM .. 15,1.`,
    linkUrl,
    `.— Aurum · TACC Pte Ltd · `,
  ].join('\n');
  ...
  const inner = `
    ${lockupRow}
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:36px 32px 8px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.34em;color:#C5A572">SIGN IN  ·  </td></tr>
...
export function buildPasswordResetEmail({ lead, resetCode }) {
  const subject = `Reset your AURUM password  ·  `;
  const text = [
    `Reset your AURUM password.Enter this 6-digit code on the reset page. It expires in 15 minutes.Reset code:  ${resetCode}If you didn't request this, ignore this email — your password is unchanged.— The PartnersAurum · TACC Pte Ltd · Singapore─────────────────────────────────────────AURUM .6. 15.:  ${resetCode}— .— Aurum · TACC Pte Ltd · `,
  ].join('\n');

  const inner = `
    ${lockupRow}
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:36px 32px 8px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.34em;color:#C5A572">PASSWORD RESET  ·  </td></tr>
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 22px;font-family:Georgia,serif;font-weight:500;font-size:30px;line-height:1.2;letter-spacing:-0.01em;color:#e8e3d8">
    ...
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 30px">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse"><tr>
        <td bgcolor="#0a0a0a" align="center" style="background:#0a0a0a;border:1px solid rgba(197,165,114,0.50);padding:22px 24px">
          <div style="font-family:'JetBrains Mono',ui-monospace,monospace;font-size:9.5px;letter-spacing:.30em;color:#8a7d6b;margin-bottom:10px">RESET CODE  ·  </div>
...
export function buildInquiryReceivedEmail({ lead }) {
  ...
  const text = [
    `Your inquiry has been received.A timestamp record of your submission is on file. A member of the team willreview and respond from a private address. On verification, next steps willbe communicated, including review of the confidentiality agreement.This email confirms the receipt of your inquiry, made of your own accord.The Aurum Century Club does not solicit publicly. All members initiatecontact in writing prior to receipt of substantive offering materials.Reference: ${lead.id}Submitted: ${submittedIso}— The PartnersAurum · TACC Pte Ltd · Singapore─────────────────────────────────────────.. . ..The Aurum Century Club. .: ${lead.id}: ${submittedIso}— Aurum · TACC Pte Ltd · `,
  ].join('\n');

  const inner = `
    ${lockupRow}

    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:36px 32px 8px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.34em;color:#C5A572">RECEIVED  ·  </td></tr>
    ...
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 36px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:10px;letter-spacing:.18em;color:#6b655e;line-height:1.8">
      : <span style="color:#C5A572">${escape(lead.id)}</span><br>
      : <span style="color:#C5A572">${escape(submittedIso)}</span>
    </td></tr>
...
export function buildNdaUploadedAlert({ lead }) {
  ...
  const text = [
    `${lead.name || 'A lead'} just uploaded their signed NDA.Name:    ${lead.name || ''}Email:   ${lead.email || ''}Code:    ${lead.code || '—'}Review and approve/reject:${adminUrl}— Aurum system`,
  ].join('\n');
...
export function buildNdaRejectedEmail({ lead, reason }) {
  const subject = `Your NDA needs revision  ·  NDA `;
  const reasonLine = reason ? `Reason: ${reason}` : '';
  const reasonLineKo = reason ? `: ${reason}` : '';
  const accessCode = lead.code || '';

  const text = [
    `Hello,The signed NDA you uploaded was not accepted.${reason ? ' ' + reasonLine : ''}Please sign a fresh copy and re-upload it through the same link.Your access code remains: ${accessCode}If anything is unclear, reply to this email and a partner will respond.— The PartnersAurum · TACC Pte Ltd · Singapore─────────────────────────────────────────,NDA.${reason ? ' ' + reasonLineKo : ''}.: ${accessCode}.— Aurum · TACC Pte Ltd · `,
  ].join('\n');

  const inner = `
    ${lockupRow}

    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:36px 32px 8px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.34em;color:#C5A572">REVISION NEEDED  ·  </td></tr>
    ...
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 14px;font-family:'Noto Serif KR',Georgia,serif;font-size:14.5px;line-height:1.85;color:#aaa39a">
      NDA${reason ? '. <span style="color:#e8e3d8">' + escape(reason) + '</span>' : '.'}
    </td></tr>
...
export function buildIoiReceivedEmail({ lead, ioi }) {
  const subject = `Your indication has been received  ·  `;
  ...
  const text = [
    ...
    `A partner will review and verify within one to two business days,then send wire instructions separately.You can track your status anytime at:https://theaurumcc.com/portfolio— The PartnersAurum · TACC Pte Ltd · Singapore─────────────────────────────────────────. .`,
    kgText ? `: ${kgText}` : '',
    krwText ? `KRW: ${krwText}` : '',
    `1–2 .:https://theaurumcc.com/portfolio— Aurum · TACC Pte Ltd · `,
  ].filter(Boolean).join('\n');
...
export function buildInvitationEmail({ lead, code, accessUrl }) {
  ...
  const text = [
    `Welcome to The Aurum Century Club.The code below opens the program. Read about the Century Club at your pace, then continue to the confidentiality agreement when ready.Access code:  ${code}Open the program:  ${accessUrl}— The PartnersAurum · TACC Pte Ltd · Singapore─────────────────────────────────────────The Aurum Century Club.. , .:    ${code}:  ${accessUrl}— Aurum · TACC Pte Ltd · `,
  ].join('\n');
  ...
  const inner = `
    ...
    <!-- Eyebrow -->
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:36px 32px 8px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.34em;color:#C5A572">INVITATION  ·  </td></tr>
    ...
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 22px">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse"><tr>
        <td bgcolor="#0a0a0a" align="center" style="background:#0a0a0a;border:1px solid rgba(197,165,114,0.50);padding:22px 24px">
          <div style="font-family:'JetBrains Mono',ui-monospace,monospace;font-size:9.5px;letter-spacing:.30em;color:#8a7d6b;margin-bottom:10px">ACCESS CODE  ·  </div>
...
export function buildMaterialsOpenEmail({ lead, accessCode, accessUrl }) {
  const subject = `The materials are open  ·  `;

  const text = [
    `Your NDA has been countersigned. The materials are open in the member portal.Access code:    ${accessCode}Open materials: ${accessUrl}Read at your pace. The path forward continues inside the portal.— The PartnersAurum · TACC Pte Ltd · Singapore─────────────────────────────────────────NDA. .:     ${accessCode}:     ${accessUrl}. .— Aurum · TACC Pte Ltd · `,
  ].join('\n');

  const inner = `
    ${lockupRow}

    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:36px 32px 8px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.34em;color:#C5A572">MATERIALS  ·  </td></tr>
...
export function buildWireInstructionsEmail({ lead, ioi, wire }) {
  const subject = `Wire instructions  ·  `;
  const ref = wire.ref;
  ...
  const text = [
    `Your IOI has been confirmed. Wire instructions follow.Beneficiary:        ${sc_beneficiary}Bank:               ${sc_bank}Account:            ${sc_account}SWIFT:              ${sc_swift}Reference:          ${ref}    ← requiredCurrency:           KRWCommitment:         ${ioi.kg.toFixed(2)} kgFinal settlement is calculated at gold spot on the day your wire clearsplus two business days to convert. The KRW required at execution maydiffer from the figure shown in the IOI form. We will confirm finalsettlement on receipt.Please complete within ${deadline_days} business days. Use the referenceabove on the wire memo so we can match it on receipt.— The PartnersAurum · TACC Pte Ltd · Singapore─────────────────────────────────────────. .:            ${sc_beneficiary}:              ${sc_bank}:          ${sc_account}SWIFT:             ${sc_swift}:          ${ref}    ← :              KRW:              ${ioi.kg.toFixed(2)} kg+ 2.KRW .${deadline_days} . .— Aurum · TACC Pte Ltd · `,
  ].join('\n');
  ...
  const inner = `
    ...
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:36px 32px 8px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.34em;color:#C5A572">WIRE INSTRUCTIONS  ·  </td></tr>
...
export function buildAdmissionEmail({ lead, magicLinkToken }) {
  const subject = `Welcome to the 100  ·  Set up your password  ·  `;
  ...
  const text = [
    `Welcome to the 100. Your wire has cleared.You're now an admitted member. Set up a password to access your portfolio:`,
    setupUrl,
    `The link above is one-time. After you set a password, the access code fromearlier emails will no longer work — your email + password is your permanentsign-in for ${siteUrl}/login.Bar serial numbers post within 5 business days as gold is allocated atSingapore Freeport. You'll be notified by email at each milestone.— The PartnersAurum · TACC Pte Ltd · Singapore─────────────────────────────────────────100. .. :`,
    setupUrl,
    `1. ,${siteUrl}/login .Freeport 5. .— Aurum · TACC Pte Ltd · `,
  ].join('\n');

  const inner = `
    ${lockupRow}

    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:36px 32px 8px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.34em;color:#C5A572">ADMITTED  ·  </td></tr>
    ...
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 22px" align="left">
      <a href="${setupUrl}" style="display:inline-block;padding:14px 26px;background:transparent;border:1px solid #C5A572;color:#C5A572;text-decoration:none;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.30em;text-transform:uppercase">→</a>
    </td></tr>
...
export function buildQuarterlyStatementEmail({ lead, period, statement_url }) {
  const subject = `Your ${period} statement is ready  ·  `;

  const text = [
    `Your ${period} statement is now available in your portfolio.Open: https://theaurumcc.com/portfolioDirect link to PDF: ${statement_url || 'in your portfolio'}Figures shown are unaudited and subject to confirmationby the fund administrator.— The PartnersAurum · TACC Pte Ltd · Singapore─────────────────────────────────────────${period} .: https://theaurumcc.com/portfolio,.— Aurum · TACC Pte Ltd · `,
  ].join('\n');
...
export function buildCapitalCallEmail({ lead, cc }) {
  ...
  const subject = `Capital Call #${cc.number}  ·  ${cc.position_code} — ${krwFmt}  ·  `;

  const text = [
    `Capital Call #${cc.number} — ${cc.position_code}Amount: ${krwFmt}Response by: ${dueText}`,
    cc.pdf_url ? `Notice PDF: ${cc.pdf_url}` : `Open your portfolio to acknowledge:https://theaurumcc.com/portfolio— The PartnersAurum · TACC Pte Ltd · Singapore─────────────────────────────────────────#${cc.number} — ${cc.position_code}: ${krwFmt}: ${dueText}:https://theaurumcc.com/portfolio— Aurum · TACC Pte Ltd · `,
  ].filter(Boolean).join('\n');

  const inner = `
    ${lockupRow}

    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:36px 32px 8px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.34em;color:#C5A572">CAPITAL CALL #${cc.number}  ·  </td></tr>
    ...
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 14px;font-family:'Noto Serif KR',Georgia,serif;font-size:14.5px;line-height:1.85;color:#aaa39a">
      <span style="color:#C5A572">${escape(cc.position_code)}</span> . ${dueText !== '—' ? `<span style="color:#e8e3d8">${escape(dueText)}</span>` : ''}.
    </td></tr>
...
export function buildWireReminderEmail({ lead, ioi, wire }) {
  const subject = `Reminder · your wire is pending  ·  `;
  const ref = wire.ref || '—';
  const kg = ioi.kg ? Number(ioi.kg).toFixed(2) : '—';

  const text = [
    `This is a friendly reminder that your wire is still pending.Wire reference: ${ref}Commitment: ${kg} kgOriginal wire instructions remain valid. If you've already sentthe wire, please disregard — clearance can take 1–2 business days.Open your portfolio:https://theaurumcc.com/portfolio— The PartnersAurum · TACC Pte Ltd · Singapore─────────────────────────────────────────. .: ${ref}: ${kg} kg. — 1–2 .:https://theaurumcc.com/portfolio— Aurum · TACC Pte Ltd · `,
  ].join('\n');

  const inner = `
    ${lockupRow}

    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:36px 32px 8px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.34em;color:#C5A572">REMINDER  ·  </td></tr>
    ...
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 14px;font-family:'Noto Serif KR',Georgia,serif;font-size:14.5px;line-height:1.85;color:#aaa39a">
      . : <span style="font-family:'JetBrains Mono',ui-monospace,monospace;color:#E3C187;letter-spacing:0.10em">${escape(ref)}</span>.
    </td></tr>
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 36px;font-family:'Noto Serif KR',Georgia,serif;font-style:italic;font-size:13px;line-height:1.7;color:#6b655e">
      — 1–2 .
    </td></tr>
...
function formatKRWInline(amount) {
  ...
  if (won) parts.push(won.toLocaleString('en-US') + '');
  return parts.join(' ') || '0';
}
...
// ── Advisor welcome / setup email ─────────────────────────────────────────────
// Sent when a partner creates a new GP advisor account. One-time setup link.
export function buildAdvisorSetupEmail({ advisor, setupUrl }) {
  const subject = `Your Aurum GP Advisor Portal access`;
  const text = [
    `Hello ${advisor.name},Your GP advisor account has been created on the Aurum Century Club portal.Set up your password to access the portal:${setupUrl}This link expires in 7 days and works once.Once set up, sign in at: ${process.env.SITE_URL || 'https://www.theaurumcc.com'}/login?r=advisor— The PartnersAurum · TACC Pte Ltd · Singapore`,
  ].join('\n');

  const inner = `
    ${lockupRow}
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:36px 32px 8px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.34em;color:#C5A572">GP ADVISOR PORTAL  ·  </td></tr>
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 22px;font-family:Georgia,serif;font-weight:500;font-size:30px;line-height:1.2;letter-spacing:-0.01em;color:#e8e3d8">
      Welcome, <span style="font-style:italic;color:#C5A572">${escape(advisor.name)}.</span>
    </td></tr>
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 22px;font-family:Georgia,serif;font-size:16px;line-height:1.78;color:#aaa39a">
      Your GP advisor account has been created for the Aurum Century Club portal. Set up your password to access the deal pipeline.
    </td></tr>
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 8px;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:9.5px;letter-spacing:.22em;color:#8a7d6b;text-transform:uppercase">Firm</td></tr>
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 22px;font-family:Georgia,serif;font-size:15px;color:#e8e3d8">${escape(advisor.firm)}</td></tr>
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 30px" align="left">
      <a href="${setupUrl}" style="display:inline-block;padding:14px 26px;background:#C5A572;color:#0a0a0a;text-decoration:none;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.30em;text-transform:uppercase;font-weight:600">SET UP ACCESS  →</a>
    </td></tr>
    <tr><td bgcolor="#0a0a0a" style="background:#0a0a0a;padding:0 32px 30px;font-family:Georgia,serif;font-style:italic;font-size:13px;line-height:1.7;color:#6b655e">
      This link expires in 7 days and works once. After setup, sign in at /login.
    </td></tr>
    ${signOffRow}
  `;
  const html = shellHtml({ inner, hidden_preheader: 'Set up your Aurum GP Advisor Portal access.' });
  return { subject, text, html };
}
