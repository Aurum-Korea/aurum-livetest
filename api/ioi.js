...
function filterMemberAudit(audit) {
  ...
  const VISIBLE = {
    code_sent: () => ({ en: 'Invitation issued', ko: ' ' }),
    code_resent: () => ({ en: 'Invitation resent', ko: ' ' }),
    nda_signed: () => ({ en: 'NDA signed', ko: 'NDA ' }),
    nda_approved: () => ({ en: 'NDA approved · materials access', ko: 'NDA  ·  ' }),
    ioi_submitted: (a) => ({
      en: `Indication signed${a.kg ? ` · ${a.kg} kg` : ''}`,
      ko: ` ${a.kg ? ` · ${a.kg} kg` : ''}`,
    }),
    ioi_verified: () => ({ en: 'Indication verified by partner', ko: '  ' }),
    wire_marked_wired: () => ({ en: 'Wire received', ko: ' ' }),
    wire_marked_cleared: () => ({ en: 'Wire cleared · admitted to cohort', ko: '  ·  ' }),
    bar_added: (a) => ({
      en: `Bar ${a.serial || ''} assigned${a.kg ? ` · ${Number(a.kg).toFixed(2)} kg` : ''}`,
      ko: ` ${a.serial || ''} ${a.kg ? ` · ${Number(a.kg).toFixed(2)} kg` : ''}`,
    }),
    doc_added: (a) => ({
    ...
    capital_call_added: (a) => ({
      ...
      ko: `  #${a.number || ''} ${a.position_code ? ` · ${a.position_code}` : ''}`,
    }),
    capital_call_acknowledged: (a) => ({
      en: `Capital call acknowledged`,
      ko: `  `,
    }),
    admission_email_sent: () => ({ en: 'Admission confirmed', ko: '  ' }),
  };
  const out = [];
