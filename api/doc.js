...
export default async function handler(req, res) {
  ...
  // Admin bypass — partners can open any bundled doc without a member session
  if (!lead) {
    const adminTokDocs = getCookie(req, 'aurum_admin');
    const adminSessDocs = verifyToken(adminTokDocs);
    if (adminSessDocs && adminSessDocs.sub === 'admin') {
      let buf2;
      try {
        const p2 = join(process.cwd(), '_docs', def.file);
        buf2 = await readFile(p2);
      } catch (e) {
        console.error('admin doc read failed', e);
    ...
    return unauthorized(res, 'no access — verify your code');
  }
