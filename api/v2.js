...
import {
  sendRaw, buildInvitationEmail, partnerBcc,
} from './_lib/email.js';
import {
...
async function handleAdvisor(req, res, op) {
  ...
  if (op === 'me') {
    ...
    return ok(res, { ok: true, advisor: safeAdvisor(auth.advisor), deals });
  }
  ...
  if (op === 'deals') {
    ...
    if (action === 'submit') {
      ...
      const deal = {
        ...
        ioi_count: 0, ioi_agg_usd: 0, gate_status: 'open',
  ...
  if (op === 'create') {
    ...
    if (!name || !firm || !email) return bad(res, 'name, firm, email required');
    if (!pw || pw.length < 8) return bad(res, 'password must be at least 8 characters');
    ...
    const adv = {
      ...
      notes: String(body.notes || '').trim(), status: 'active',
      created_at: now, created_by: adminSession.id || 'partner',
      last_login_at: null, updated_at: now,
    };
    try { await saveAdvisor(adv); return ok(res, { ok: true, advisor: safeAdvisor(adv) }); }
    catch (e) { return serverError(res, 'create failed'); }
  }
