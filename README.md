...
- A **partners-only dashboard** (`/admin`) for two principals to review sign-ups,
  add notes, generate invitation codes, and send branded bilingual emails.
...
Everything lives inside `/api/` (Vercel serverless functions) and three new
...
ADMIN_PASSWORD           ← single shared password (use this OR ADMIN_PASSWORDS)
ADMIN_PASSWORDS          ← comma-separated, one per partner.
                           example: ChangMin2026!,Sora2026!
                           login returns the partner index (1 or 2) so the
                           audit trail credits actions to the right person.
```

You can use **either** `ADMIN_PASSWORD` (one shared) **or** `ADMIN_PASSWORDS`
(per-partner). If both are set, `ADMIN_PASSWORDS` wins.
...
### 3. Set the secret + admin password

```
AURUM_SECRET     = <output of: openssl rand -hex 32>
ADMIN_PASSWORDS  = partner1Pass,partner2Pass
```

Pick strong passwords. Anyone with one can read every sign-up.
...
Visit `https://theaurumcc.com/admin`. Enter a password. You're in.
...
| `api/login.js` | Checks password, sets `aurum_admin` cookie (12h). |
| `api/logout.js` | Clears the admin cookie. |
| `api/me.js` | Returns current session info or 401. |
...
| `api/_lib/auth.js` | HMAC sign/verify, password check, code generator. |
...
- **"new sign-ups not appearing"** — `KV_REST_API_URL` / `KV_REST_API_TOKEN`
  not set, so writes land in the in-memory dev store and vanish on the
  next cold start. Connect Upstash via the Marketplace.
